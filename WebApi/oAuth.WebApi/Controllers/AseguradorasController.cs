...
namespace Spartane.WebApi.Controllers
{
    ...
    public partial class AseguradorasController : BaseApiController
    {
        #region Variables
        private IAseguradorasService service = null;
        public  static string ApiControllerUrl { get; set; }
		private ISpartan_Bitacora_SQLService serviceBitacora = null;
		private static int object_id = 44422;
        public string baseApi;
        #endregion Variables

        #region Constructor
        //TODO: Global Data y Data Rerence tienen que estar en un controlador base
        public AseguradorasController(IAseguradorasService service,ISpartan_Bitacora_SQLService bitacoraService)
        {
        ...
        [Authorize]
        [HttpPut]
        public HttpResponseMessage Put(int id, Aseguradoras varAseguradoras)
        {
            if (ModelState.IsValid && id == varAseguradoras.Folio)
            {
                var data = "-1";
                try
                {
                    data = Convert.ToString(this.service.Update(varAseguradoras));
					 var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdAseguradoras", new JavaScriptSerializer().Serialize(varAseguradoras), true);
                    serviceBitacora.Insert(bitacora);
                }
                catch (ServiceException ex)
                {
				    var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdAseguradoras", new JavaScriptSerializer().Serialize(varAseguradoras), false, ex.Message);
                    serviceBitacora.Insert(bitacora);
                    return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
                }
            ...
            {
               var errors= ModelState.SelectMany(m => m.Value.Errors.Select(err => err.ErrorMessage != string.Empty ? err.ErrorMessage : err.Exception.Message).ToList()).ToList();                
               var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdAseguradoras", new JavaScriptSerializer().Serialize(varAseguradoras), false, String.Join(",", errors.ToArray()));
               serviceBitacora.Insert(bitacora);
               return Request.CreateResponse(HttpStatusCode.BadRequest, errors);
            }
        }
...
result.Nombre = entity.Nombre;
	...
	[Authorize]
        ...
        public HttpResponseMessage GetAllTunnel(string user, string password)
        {
                ...
                var AseguradorasDataTable = new JavaScriptSerializer().Deserialize<List<Aseguradoras>>(result);
                return Request.CreateResponse(HttpStatusCode.OK, AseguradorasDataTable, Configuration.Formatters.JsonFormatter);
        }
        ...
        public HttpResponseMessage GetTunnel(int id,string user, string password)
        {
                ...
                var AseguradorasResult = new JavaScriptSerializer().Deserialize<Aseguradoras>(result);
                return Request.CreateResponse(HttpStatusCode.OK, AseguradorasResult, Configuration.Formatters.JsonFormatter);            
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage PostTunnel(Aseguradoras emp,string user, string password)
        {
                var client = new System.Net.WebClient();
                client.Headers = TokenManager.GetAuthenticationHeader(user,password);

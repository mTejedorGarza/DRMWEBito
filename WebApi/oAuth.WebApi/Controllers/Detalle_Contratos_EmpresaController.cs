...
namespace Spartane.WebApi.Controllers
{
    ...
    public partial class Detalle_Contratos_EmpresaController : BaseApiController
    {
        #region Variables
        private IDetalle_Contratos_EmpresaService service = null;
        public  static string ApiControllerUrl { get; set; }
		private ISpartan_Bitacora_SQLService serviceBitacora = null;
		private static int object_id = 44413;
        public string baseApi;
        #endregion Variables

        #region Constructor
        //TODO: Global Data y Data Rerence tienen que estar en un controlador base
        public Detalle_Contratos_EmpresaController(IDetalle_Contratos_EmpresaService service,ISpartan_Bitacora_SQLService bitacoraService)
        {
        ...
        [Authorize]
        [HttpPut]
        public HttpResponseMessage Put(int id, Detalle_Contratos_Empresa varDetalle_Contratos_Empresa)
        {
            if (ModelState.IsValid && id == varDetalle_Contratos_Empresa.Folio)
            {
                var data = "-1";
                try
                {
                    data = Convert.ToString(this.service.Update(varDetalle_Contratos_Empresa));
					 var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdDetalle_Contratos_Empresa", new JavaScriptSerializer().Serialize(varDetalle_Contratos_Empresa), true);
                    serviceBitacora.Insert(bitacora);
                }
                catch (ServiceException ex)
                {
				    var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdDetalle_Contratos_Empresa", new JavaScriptSerializer().Serialize(varDetalle_Contratos_Empresa), false, ex.Message);
                    serviceBitacora.Insert(bitacora);
                    return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
                }
            ...
            {
               var errors= ModelState.SelectMany(m => m.Value.Errors.Select(err => err.ErrorMessage != string.Empty ? err.ErrorMessage : err.Exception.Message).ToList()).ToList();                
               var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdDetalle_Contratos_Empresa", new JavaScriptSerializer().Serialize(varDetalle_Contratos_Empresa), false, String.Join(",", errors.ToArray()));
               serviceBitacora.Insert(bitacora);
               return Request.CreateResponse(HttpStatusCode.BadRequest, errors);
            }
        }
...
result.Fecha_de_Firma_de_Contrato = entity.Fecha_de_Firma_de_Contrato;
	...
	[Authorize]
        ...
        public HttpResponseMessage GetAllTunnel(string user, string password)
        {
                ...
                var Detalle_Contratos_EmpresaDataTable = new JavaScriptSerializer().Deserialize<List<Detalle_Contratos_Empresa>>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Detalle_Contratos_EmpresaDataTable, Configuration.Formatters.JsonFormatter);
        }
        ...
        public HttpResponseMessage GetTunnel(int id,string user, string password)
        {
                ...
                var Detalle_Contratos_EmpresaResult = new JavaScriptSerializer().Deserialize<Detalle_Contratos_Empresa>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Detalle_Contratos_EmpresaResult, Configuration.Formatters.JsonFormatter);            
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage PostTunnel(Detalle_Contratos_Empresa emp,string user, string password)
        {
                var client = new System.Net.WebClient();
                client.Headers = TokenManager.GetAuthenticationHeader(user,password);

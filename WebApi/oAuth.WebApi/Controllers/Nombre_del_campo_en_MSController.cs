...
namespace Spartane.WebApi.Controllers
{
    ...
    public partial class Nombre_del_campo_en_MSController : BaseApiController
    {
        #region Variables
        private INombre_del_campo_en_MSService service = null;
        public  static string ApiControllerUrl { get; set; }
		private ISpartan_Bitacora_SQLService serviceBitacora = null;
		private static int object_id = 44706;
        public string baseApi;
        #endregion Variables

        #region Constructor
        //TODO: Global Data y Data Rerence tienen que estar en un controlador base
        public Nombre_del_campo_en_MSController(INombre_del_campo_en_MSService service,ISpartan_Bitacora_SQLService bitacoraService)
        {
        ...
        [Authorize]
        [HttpPut]
        public HttpResponseMessage Put(int id, Nombre_del_campo_en_MS varNombre_del_campo_en_MS)
        {
            if (ModelState.IsValid && id == varNombre_del_campo_en_MS.Clave)
            {
                var data = "-1";
                try
                {
                    data = Convert.ToString(this.service.Update(varNombre_del_campo_en_MS));
					 var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdNombre_del_campo_en_MS", new JavaScriptSerializer().Serialize(varNombre_del_campo_en_MS), true);
                    serviceBitacora.Insert(bitacora);
                }
                catch (ServiceException ex)
                {
				    var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdNombre_del_campo_en_MS", new JavaScriptSerializer().Serialize(varNombre_del_campo_en_MS), false, ex.Message);
                    serviceBitacora.Insert(bitacora);
                    return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
                }
            ...
            {
               var errors= ModelState.SelectMany(m => m.Value.Errors.Select(err => err.ErrorMessage != string.Empty ? err.ErrorMessage : err.Exception.Message).ToList()).ToList();                
               var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdNombre_del_campo_en_MS", new JavaScriptSerializer().Serialize(varNombre_del_campo_en_MS), false, String.Join(",", errors.ToArray()));
               serviceBitacora.Insert(bitacora);
               return Request.CreateResponse(HttpStatusCode.BadRequest, errors);
            }
        }
...
result.Nombre_Fisico_de_la_Tabla = entity.Nombre_Fisico_de_la_Tabla;
	...
	[Authorize]
        ...
        public HttpResponseMessage GetAllTunnel(string user, string password)
        {
                ...
                var Nombre_del_campo_en_MSDataTable = new JavaScriptSerializer().Deserialize<List<Nombre_del_campo_en_MS>>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Nombre_del_campo_en_MSDataTable, Configuration.Formatters.JsonFormatter);
        }
        ...
        public HttpResponseMessage GetTunnel(int id,string user, string password)
        {
                ...
                var Nombre_del_campo_en_MSResult = new JavaScriptSerializer().Deserialize<Nombre_del_campo_en_MS>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Nombre_del_campo_en_MSResult, Configuration.Formatters.JsonFormatter);            
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage PostTunnel(Nombre_del_campo_en_MS emp,string user, string password)
        {
                var client = new System.Net.WebClient();
                client.Headers = TokenManager.GetAuthenticationHeader(user,password);

...
namespace Spartane.WebApi.Controllers
{
    ...
    public partial class areas_EmpresasController : BaseApiController
    {
        #region Variables
        private Iareas_EmpresasService service = null;
        public  static string ApiControllerUrl { get; set; }
		private ISpartan_Bitacora_SQLService serviceBitacora = null;
		private static int object_id = 44409;
        public string baseApi;
        #endregion Variables

        #region Constructor
        //TODO: Global Data y Data Rerence tienen que estar en un controlador base
        public areas_EmpresasController(Iareas_EmpresasService service,ISpartan_Bitacora_SQLService bitacoraService)
        {
        ...
        [Authorize]
        [HttpPut]
        public HttpResponseMessage Put(int id, areas_Empresas varareas_Empresas)
        {
            if (ModelState.IsValid && id == varareas_Empresas.Clave)
            {
                var data = "-1";
                try
                {
                    data = Convert.ToString(this.service.Update(varareas_Empresas));
					 var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_Updareas_Empresas", new JavaScriptSerializer().Serialize(varareas_Empresas), true);
                    serviceBitacora.Insert(bitacora);
                }
                catch (ServiceException ex)
                {
				    var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_Updareas_Empresas", new JavaScriptSerializer().Serialize(varareas_Empresas), false, ex.Message);
                    serviceBitacora.Insert(bitacora);
                    return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
                }
            ...
            {
               var errors= ModelState.SelectMany(m => m.Value.Errors.Select(err => err.ErrorMessage != string.Empty ? err.ErrorMessage : err.Exception.Message).ToList()).ToList();                
               var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_Updareas_Empresas", new JavaScriptSerializer().Serialize(varareas_Empresas), false, String.Join(",", errors.ToArray()));
               serviceBitacora.Insert(bitacora);
               return Request.CreateResponse(HttpStatusCode.BadRequest, errors);
            }
        }
...
result.Nombre = entity.Nombre;
	...
	[Authorize]
        ...
        public HttpResponseMessage GetAllTunnel(string user, string password)
        {
                ...
                var areas_EmpresasDataTable = new JavaScriptSerializer().Deserialize<List<areas_Empresas>>(result);
                return Request.CreateResponse(HttpStatusCode.OK, areas_EmpresasDataTable, Configuration.Formatters.JsonFormatter);
        }
        ...
        public HttpResponseMessage GetTunnel(int id,string user, string password)
        {
                ...
                var areas_EmpresasResult = new JavaScriptSerializer().Deserialize<areas_Empresas>(result);
                return Request.CreateResponse(HttpStatusCode.OK, areas_EmpresasResult, Configuration.Formatters.JsonFormatter);            
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage PostTunnel(areas_Empresas emp,string user, string password)
        {
                var client = new System.Net.WebClient();
                client.Headers = TokenManager.GetAuthenticationHeader(user,password);

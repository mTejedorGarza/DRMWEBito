...
namespace Spartane.WebApi.Controllers
{
    ...
    public partial class Tipo_de_RutinaController : BaseApiController
    {
        #region Variables
        private ITipo_de_RutinaService service = null;
        public  static string ApiControllerUrl { get; set; }
		private ISpartan_Bitacora_SQLService serviceBitacora = null;
		private static int object_id = 44609;
        public string baseApi;
        #endregion Variables

        #region Constructor
        //TODO: Global Data y Data Rerence tienen que estar en un controlador base
        public Tipo_de_RutinaController(ITipo_de_RutinaService service,ISpartan_Bitacora_SQLService bitacoraService)
        {
        ...
        [Authorize]
        [HttpPut]
        public HttpResponseMessage Put(int id, Tipo_de_Rutina varTipo_de_Rutina)
        {
            if (ModelState.IsValid && id == varTipo_de_Rutina.Folio)
            {
                var data = "-1";
                try
                {
                    data = Convert.ToString(this.service.Update(varTipo_de_Rutina));
					 var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdTipo_de_Rutina", new JavaScriptSerializer().Serialize(varTipo_de_Rutina), true);
                    serviceBitacora.Insert(bitacora);
                }
                catch (ServiceException ex)
                {
				    var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdTipo_de_Rutina", new JavaScriptSerializer().Serialize(varTipo_de_Rutina), false, ex.Message);
                    serviceBitacora.Insert(bitacora);
                    return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
                }
            ...
            {
               var errors= ModelState.SelectMany(m => m.Value.Errors.Select(err => err.ErrorMessage != string.Empty ? err.ErrorMessage : err.Exception.Message).ToList()).ToList();                
               var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdTipo_de_Rutina", new JavaScriptSerializer().Serialize(varTipo_de_Rutina), false, String.Join(",", errors.ToArray()));
               serviceBitacora.Insert(bitacora);
               return Request.CreateResponse(HttpStatusCode.BadRequest, errors);
            }
        }
...
result.Descripcion = entity.Descripcion;
	...
	[Authorize]
        ...
        public HttpResponseMessage GetAllTunnel(string user, string password)
        {
                ...
                var Tipo_de_RutinaDataTable = new JavaScriptSerializer().Deserialize<List<Tipo_de_Rutina>>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Tipo_de_RutinaDataTable, Configuration.Formatters.JsonFormatter);
        }
        ...
        public HttpResponseMessage GetTunnel(int id,string user, string password)
        {
                ...
                var Tipo_de_RutinaResult = new JavaScriptSerializer().Deserialize<Tipo_de_Rutina>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Tipo_de_RutinaResult, Configuration.Formatters.JsonFormatter);            
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage PostTunnel(Tipo_de_Rutina emp,string user, string password)
        {
                var client = new System.Net.WebClient();
                client.Headers = TokenManager.GetAuthenticationHeader(user,password);

...
namespace Spartane.WebApi.Controllers
{
    ...
    public partial class Detalle_Actividades_EventoController : BaseApiController
    {
        #region Variables
        private IDetalle_Actividades_EventoService service = null;
        public  static string ApiControllerUrl { get; set; }
		private ISpartan_Bitacora_SQLService serviceBitacora = null;
		private static int object_id = 44431;
        public string baseApi;
        #endregion Variables

        #region Constructor
        //TODO: Global Data y Data Rerence tienen que estar en un controlador base
        public Detalle_Actividades_EventoController(IDetalle_Actividades_EventoService service,ISpartan_Bitacora_SQLService bitacoraService)
        {
        ...
        [Authorize]
        [HttpPut]
        public HttpResponseMessage Put(int id, Detalle_Actividades_Evento varDetalle_Actividades_Evento)
        {
            if (ModelState.IsValid && id == varDetalle_Actividades_Evento.Folio)
            {
                var data = "-1";
                try
                {
                    data = Convert.ToString(this.service.Update(varDetalle_Actividades_Evento));
					 var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdDetalle_Actividades_Evento", new JavaScriptSerializer().Serialize(varDetalle_Actividades_Evento), true);
                    serviceBitacora.Insert(bitacora);
                }
                catch (ServiceException ex)
                {
				    var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdDetalle_Actividades_Evento", new JavaScriptSerializer().Serialize(varDetalle_Actividades_Evento), false, ex.Message);
                    serviceBitacora.Insert(bitacora);
                    return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
                }
            ...
            {
               var errors= ModelState.SelectMany(m => m.Value.Errors.Select(err => err.ErrorMessage != string.Empty ? err.ErrorMessage : err.Exception.Message).ToList()).ToList();                
               var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdDetalle_Actividades_Evento", new JavaScriptSerializer().Serialize(varDetalle_Actividades_Evento), false, String.Join(",", errors.ToArray()));
               serviceBitacora.Insert(bitacora);
               return Request.CreateResponse(HttpStatusCode.BadRequest, errors);
            }
        }
...
result.Estatus_de_la_Actividad_Estatus_Actividades_Evento = entity.Estatus_de_la_Actividad_Estatus_Actividades_Evento;
	...
	[Authorize]
        ...
        public HttpResponseMessage GetAllTunnel(string user, string password)
        {
                ...
                var Detalle_Actividades_EventoDataTable = new JavaScriptSerializer().Deserialize<List<Detalle_Actividades_Evento>>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Detalle_Actividades_EventoDataTable, Configuration.Formatters.JsonFormatter);
        }
        ...
        public HttpResponseMessage GetTunnel(int id,string user, string password)
        {
                ...
                var Detalle_Actividades_EventoResult = new JavaScriptSerializer().Deserialize<Detalle_Actividades_Evento>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Detalle_Actividades_EventoResult, Configuration.Formatters.JsonFormatter);            
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage PostTunnel(Detalle_Actividades_Evento emp,string user, string password)
        {
                var client = new System.Net.WebClient();
                client.Headers = TokenManager.GetAuthenticationHeader(user,password);

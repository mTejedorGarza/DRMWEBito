...
namespace Spartane.WebApi.Controllers
{
    ...
    public partial class ProfesionesController : BaseApiController
    {
        #region Variables
        private IProfesionesService service = null;
        public  static string ApiControllerUrl { get; set; }
		private ISpartan_Bitacora_SQLService serviceBitacora = null;
		private static int object_id = 44377;
        public string baseApi;
        #endregion Variables

        #region Constructor
        //TODO: Global Data y Data Rerence tienen que estar en un controlador base
        public ProfesionesController(IProfesionesService service,ISpartan_Bitacora_SQLService bitacoraService)
        {
        ...
        [Authorize]
        [HttpPut]
        public HttpResponseMessage Put(int id, Profesiones varProfesiones)
        {
            if (ModelState.IsValid && id == varProfesiones.Clave)
            {
                var data = "-1";
                try
                {
                    data = Convert.ToString(this.service.Update(varProfesiones));
					 var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdProfesiones", new JavaScriptSerializer().Serialize(varProfesiones), true);
                    serviceBitacora.Insert(bitacora);
                }
                catch (ServiceException ex)
                {
				    var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdProfesiones", new JavaScriptSerializer().Serialize(varProfesiones), false, ex.Message);
                    serviceBitacora.Insert(bitacora);
                    return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
                }
            ...
            {
               var errors= ModelState.SelectMany(m => m.Value.Errors.Select(err => err.ErrorMessage != string.Empty ? err.ErrorMessage : err.Exception.Message).ToList()).ToList();                
               var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdProfesiones", new JavaScriptSerializer().Serialize(varProfesiones), false, String.Join(",", errors.ToArray()));
               serviceBitacora.Insert(bitacora);
               return Request.CreateResponse(HttpStatusCode.BadRequest, errors);
            }
        }
...
result.Descripcion = entity.Descripcion;
	...
	[Authorize]
        ...
        public HttpResponseMessage GetAllTunnel(string user, string password)
        {
                ...
                var ProfesionesDataTable = new JavaScriptSerializer().Deserialize<List<Profesiones>>(result);
                return Request.CreateResponse(HttpStatusCode.OK, ProfesionesDataTable, Configuration.Formatters.JsonFormatter);
        }
        ...
        public HttpResponseMessage GetTunnel(int id,string user, string password)
        {
                ...
                var ProfesionesResult = new JavaScriptSerializer().Deserialize<Profesiones>(result);
                return Request.CreateResponse(HttpStatusCode.OK, ProfesionesResult, Configuration.Formatters.JsonFormatter);            
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage PostTunnel(Profesiones emp,string user, string password)
        {
                var client = new System.Net.WebClient();
                client.Headers = TokenManager.GetAuthenticationHeader(user,password);

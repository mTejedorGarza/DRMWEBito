...
namespace Spartane.WebApi.Controllers
{
    ...
    public partial class Detalle_Preferencia_BebidasController : BaseApiController
    {
        #region Variables
        private IDetalle_Preferencia_BebidasService service = null;
        public  static string ApiControllerUrl { get; set; }
		private ISpartan_Bitacora_SQLService serviceBitacora = null;
		private static int object_id = 44336;
        public string baseApi;
        #endregion Variables

        #region Constructor
        //TODO: Global Data y Data Rerence tienen que estar en un controlador base
        public Detalle_Preferencia_BebidasController(IDetalle_Preferencia_BebidasService service,ISpartan_Bitacora_SQLService bitacoraService)
        {
        ...
        [Authorize]
        [HttpPut]
        public HttpResponseMessage Put(int id, Detalle_Preferencia_Bebidas varDetalle_Preferencia_Bebidas)
        {
            if (ModelState.IsValid && id == varDetalle_Preferencia_Bebidas.Folio)
            {
                var data = "-1";
                try
                {
                    data = Convert.ToString(this.service.Update(varDetalle_Preferencia_Bebidas));
					 var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdDetalle_Preferencia_Bebidas", new JavaScriptSerializer().Serialize(varDetalle_Preferencia_Bebidas), true);
                    serviceBitacora.Insert(bitacora);
                }
                catch (ServiceException ex)
                {
				    var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdDetalle_Preferencia_Bebidas", new JavaScriptSerializer().Serialize(varDetalle_Preferencia_Bebidas), false, ex.Message);
                    serviceBitacora.Insert(bitacora);
                    return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
                }
            ...
            {
               var errors= ModelState.SelectMany(m => m.Value.Errors.Select(err => err.ErrorMessage != string.Empty ? err.ErrorMessage : err.Exception.Message).ToList()).ToList();                
               var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdDetalle_Preferencia_Bebidas", new JavaScriptSerializer().Serialize(varDetalle_Preferencia_Bebidas), false, String.Join(",", errors.ToArray()));
               serviceBitacora.Insert(bitacora);
               return Request.CreateResponse(HttpStatusCode.BadRequest, errors);
            }
        }
...
result.Cantidad_Rango_Consumo_Bebidas = entity.Cantidad_Rango_Consumo_Bebidas;
	...
	[Authorize]
        ...
        public HttpResponseMessage GetAllTunnel(string user, string password)
        {
                ...
                var Detalle_Preferencia_BebidasDataTable = new JavaScriptSerializer().Deserialize<List<Detalle_Preferencia_Bebidas>>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Detalle_Preferencia_BebidasDataTable, Configuration.Formatters.JsonFormatter);
        }
        ...
        public HttpResponseMessage GetTunnel(int id,string user, string password)
        {
                ...
                var Detalle_Preferencia_BebidasResult = new JavaScriptSerializer().Deserialize<Detalle_Preferencia_Bebidas>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Detalle_Preferencia_BebidasResult, Configuration.Formatters.JsonFormatter);            
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage PostTunnel(Detalle_Preferencia_Bebidas emp,string user, string password)
        {
                var client = new System.Net.WebClient();
                client.Headers = TokenManager.GetAuthenticationHeader(user,password);

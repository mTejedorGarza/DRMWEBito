...
namespace Spartane.WebApi.Controllers
{
    ...
    public partial class MS_Semanas_PlanesController : BaseApiController
    {
        #region Variables
        private IMS_Semanas_PlanesService service = null;
        public  static string ApiControllerUrl { get; set; }
		private ISpartan_Bitacora_SQLService serviceBitacora = null;
		private static int object_id = 44602;
        public string baseApi;
        #endregion Variables

        #region Constructor
        //TODO: Global Data y Data Rerence tienen que estar en un controlador base
        public MS_Semanas_PlanesController(IMS_Semanas_PlanesService service,ISpartan_Bitacora_SQLService bitacoraService)
        {
        ...
        [Authorize]
        [HttpPut]
        public HttpResponseMessage Put(int id, MS_Semanas_Planes varMS_Semanas_Planes)
        {
            if (ModelState.IsValid && id == varMS_Semanas_Planes.Folio)
            {
                var data = "-1";
                try
                {
                    data = Convert.ToString(this.service.Update(varMS_Semanas_Planes));
					 var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdMS_Semanas_Planes", new JavaScriptSerializer().Serialize(varMS_Semanas_Planes), true);
                    serviceBitacora.Insert(bitacora);
                }
                catch (ServiceException ex)
                {
				    var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdMS_Semanas_Planes", new JavaScriptSerializer().Serialize(varMS_Semanas_Planes), false, ex.Message);
                    serviceBitacora.Insert(bitacora);
                    return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
                }
            ...
            {
               var errors= ModelState.SelectMany(m => m.Value.Errors.Select(err => err.ErrorMessage != string.Empty ? err.ErrorMessage : err.Exception.Message).ToList()).ToList();                
               var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdMS_Semanas_Planes", new JavaScriptSerializer().Serialize(varMS_Semanas_Planes), false, String.Join(",", errors.ToArray()));
               serviceBitacora.Insert(bitacora);
               return Request.CreateResponse(HttpStatusCode.BadRequest, errors);
            }
        }
...
result.Semanas_Semanas_Planes = entity.Semanas_Semanas_Planes;
	...
	[Authorize]
        ...
        public HttpResponseMessage GetAllTunnel(string user, string password)
        {
                ...
                var MS_Semanas_PlanesDataTable = new JavaScriptSerializer().Deserialize<List<MS_Semanas_Planes>>(result);
                return Request.CreateResponse(HttpStatusCode.OK, MS_Semanas_PlanesDataTable, Configuration.Formatters.JsonFormatter);
        }
        ...
        public HttpResponseMessage GetTunnel(int id,string user, string password)
        {
                ...
                var MS_Semanas_PlanesResult = new JavaScriptSerializer().Deserialize<MS_Semanas_Planes>(result);
                return Request.CreateResponse(HttpStatusCode.OK, MS_Semanas_PlanesResult, Configuration.Formatters.JsonFormatter);            
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage PostTunnel(MS_Semanas_Planes emp,string user, string password)
        {
                var client = new System.Net.WebClient();
                client.Headers = TokenManager.GetAuthenticationHeader(user,password);

...
namespace Spartane.WebApi.Controllers
{
    ...
    public partial class MS_Dificultad_PlatillosController : BaseApiController
    {
        #region Variables
        private IMS_Dificultad_PlatillosService service = null;
        public  static string ApiControllerUrl { get; set; }
		private ISpartan_Bitacora_SQLService serviceBitacora = null;
		private static int object_id = 44732;
        public string baseApi;
        #endregion Variables

        #region Constructor
        //TODO: Global Data y Data Rerence tienen que estar en un controlador base
        public MS_Dificultad_PlatillosController(IMS_Dificultad_PlatillosService service,ISpartan_Bitacora_SQLService bitacoraService)
        {
        ...
        [Authorize]
        [HttpPut]
        public HttpResponseMessage Put(int id, MS_Dificultad_Platillos varMS_Dificultad_Platillos)
        {
            if (ModelState.IsValid && id == varMS_Dificultad_Platillos.Folio)
            {
                var data = "-1";
                try
                {
                    data = Convert.ToString(this.service.Update(varMS_Dificultad_Platillos));
					 var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdMS_Dificultad_Platillos", new JavaScriptSerializer().Serialize(varMS_Dificultad_Platillos), true);
                    serviceBitacora.Insert(bitacora);
                }
                catch (ServiceException ex)
                {
				    var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdMS_Dificultad_Platillos", new JavaScriptSerializer().Serialize(varMS_Dificultad_Platillos), false, ex.Message);
                    serviceBitacora.Insert(bitacora);
                    return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
                }
            ...
            {
               var errors= ModelState.SelectMany(m => m.Value.Errors.Select(err => err.ErrorMessage != string.Empty ? err.ErrorMessage : err.Exception.Message).ToList()).ToList();                
               var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdMS_Dificultad_Platillos", new JavaScriptSerializer().Serialize(varMS_Dificultad_Platillos), false, String.Join(",", errors.ToArray()));
               serviceBitacora.Insert(bitacora);
               return Request.CreateResponse(HttpStatusCode.BadRequest, errors);
            }
        }
...
result.Dificultad_Dificultad_de_platillos = entity.Dificultad_Dificultad_de_platillos;
	...
	[Authorize]
        ...
        public HttpResponseMessage GetAllTunnel(string user, string password)
        {
                ...
                var MS_Dificultad_PlatillosDataTable = new JavaScriptSerializer().Deserialize<List<MS_Dificultad_Platillos>>(result);
                return Request.CreateResponse(HttpStatusCode.OK, MS_Dificultad_PlatillosDataTable, Configuration.Formatters.JsonFormatter);
        }
        ...
        public HttpResponseMessage GetTunnel(int id,string user, string password)
        {
                ...
                var MS_Dificultad_PlatillosResult = new JavaScriptSerializer().Deserialize<MS_Dificultad_Platillos>(result);
                return Request.CreateResponse(HttpStatusCode.OK, MS_Dificultad_PlatillosResult, Configuration.Formatters.JsonFormatter);            
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage PostTunnel(MS_Dificultad_Platillos emp,string user, string password)
        {
                var client = new System.Net.WebClient();
                client.Headers = TokenManager.GetAuthenticationHeader(user,password);

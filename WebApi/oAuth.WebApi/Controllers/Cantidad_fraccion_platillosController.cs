...
namespace Spartane.WebApi.Controllers
{
    ...
    public partial class Cantidad_fraccion_platillosController : BaseApiController
    {
        #region Variables
        private ICantidad_fraccion_platillosService service = null;
        public  static string ApiControllerUrl { get; set; }
		private ISpartan_Bitacora_SQLService serviceBitacora = null;
		private static int object_id = 44554;
        public string baseApi;
        #endregion Variables

        #region Constructor
        //TODO: Global Data y Data Rerence tienen que estar en un controlador base
        public Cantidad_fraccion_platillosController(ICantidad_fraccion_platillosService service,ISpartan_Bitacora_SQLService bitacoraService)
        {
        ...
        [Authorize]
        [HttpPut]
        public HttpResponseMessage Put(int id, Cantidad_fraccion_platillos varCantidad_fraccion_platillos)
        {
            if (ModelState.IsValid && id == varCantidad_fraccion_platillos.Folio)
            {
                var data = "-1";
                try
                {
                    data = Convert.ToString(this.service.Update(varCantidad_fraccion_platillos));
					 var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdCantidad_fraccion_platillos", new JavaScriptSerializer().Serialize(varCantidad_fraccion_platillos), true);
                    serviceBitacora.Insert(bitacora);
                }
                catch (ServiceException ex)
                {
				    var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdCantidad_fraccion_platillos", new JavaScriptSerializer().Serialize(varCantidad_fraccion_platillos), false, ex.Message);
                    serviceBitacora.Insert(bitacora);
                    return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
                }
            ...
            {
               var errors= ModelState.SelectMany(m => m.Value.Errors.Select(err => err.ErrorMessage != string.Empty ? err.ErrorMessage : err.Exception.Message).ToList()).ToList();                
               var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdCantidad_fraccion_platillos", new JavaScriptSerializer().Serialize(varCantidad_fraccion_platillos), false, String.Join(",", errors.ToArray()));
               serviceBitacora.Insert(bitacora);
               return Request.CreateResponse(HttpStatusCode.BadRequest, errors);
            }
        }
...
result.Cantidad = entity.Cantidad;
	...
	[Authorize]
        ...
        public HttpResponseMessage GetAllTunnel(string user, string password)
        {
                ...
                var Cantidad_fraccion_platillosDataTable = new JavaScriptSerializer().Deserialize<List<Cantidad_fraccion_platillos>>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Cantidad_fraccion_platillosDataTable, Configuration.Formatters.JsonFormatter);
        }
        ...
        public HttpResponseMessage GetTunnel(int id,string user, string password)
        {
                ...
                var Cantidad_fraccion_platillosResult = new JavaScriptSerializer().Deserialize<Cantidad_fraccion_platillos>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Cantidad_fraccion_platillosResult, Configuration.Formatters.JsonFormatter);            
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage PostTunnel(Cantidad_fraccion_platillos emp,string user, string password)
        {
                var client = new System.Net.WebClient();
                client.Headers = TokenManager.GetAuthenticationHeader(user,password);

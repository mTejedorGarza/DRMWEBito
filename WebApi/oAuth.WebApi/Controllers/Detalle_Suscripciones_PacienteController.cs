...
namespace Spartane.WebApi.Controllers
{
    ...
    public partial class Detalle_Suscripciones_PacienteController : BaseApiController
    {
        #region Variables
        private IDetalle_Suscripciones_PacienteService service = null;
        public  static string ApiControllerUrl { get; set; }
		private ISpartan_Bitacora_SQLService serviceBitacora = null;
		private static int object_id = 44398;
        public string baseApi;
        #endregion Variables

        #region Constructor
        //TODO: Global Data y Data Rerence tienen que estar en un controlador base
        public Detalle_Suscripciones_PacienteController(IDetalle_Suscripciones_PacienteService service,ISpartan_Bitacora_SQLService bitacoraService)
        {
        ...
        [Authorize]
        [HttpPut]
        public HttpResponseMessage Put(int id, Detalle_Suscripciones_Paciente varDetalle_Suscripciones_Paciente)
        {
            if (ModelState.IsValid && id == varDetalle_Suscripciones_Paciente.Folio)
            {
                var data = "-1";
                try
                {
                    data = Convert.ToString(this.service.Update(varDetalle_Suscripciones_Paciente));
					 var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdDetalle_Suscripciones_Paciente", new JavaScriptSerializer().Serialize(varDetalle_Suscripciones_Paciente), true);
                    serviceBitacora.Insert(bitacora);
                }
                catch (ServiceException ex)
                {
				    var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdDetalle_Suscripciones_Paciente", new JavaScriptSerializer().Serialize(varDetalle_Suscripciones_Paciente), false, ex.Message);
                    serviceBitacora.Insert(bitacora);
                    return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
                }
            ...
            {
               var errors= ModelState.SelectMany(m => m.Value.Errors.Select(err => err.ErrorMessage != string.Empty ? err.ErrorMessage : err.Exception.Message).ToList()).ToList();                
               var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdDetalle_Suscripciones_Paciente", new JavaScriptSerializer().Serialize(varDetalle_Suscripciones_Paciente), false, String.Join(",", errors.ToArray()));
               serviceBitacora.Insert(bitacora);
               return Request.CreateResponse(HttpStatusCode.BadRequest, errors);
            }
        }
...
result.Estatus_Estatus_de_Suscripcion = entity.Estatus_Estatus_de_Suscripcion;
	...
	[Authorize]
        ...
        public HttpResponseMessage GetAllTunnel(string user, string password)
        {
                ...
                var Detalle_Suscripciones_PacienteDataTable = new JavaScriptSerializer().Deserialize<List<Detalle_Suscripciones_Paciente>>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Detalle_Suscripciones_PacienteDataTable, Configuration.Formatters.JsonFormatter);
        }
        ...
        public HttpResponseMessage GetTunnel(int id,string user, string password)
        {
                ...
                var Detalle_Suscripciones_PacienteResult = new JavaScriptSerializer().Deserialize<Detalle_Suscripciones_Paciente>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Detalle_Suscripciones_PacienteResult, Configuration.Formatters.JsonFormatter);            
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage PostTunnel(Detalle_Suscripciones_Paciente emp,string user, string password)
        {
                var client = new System.Net.WebClient();
                client.Headers = TokenManager.GetAuthenticationHeader(user,password);

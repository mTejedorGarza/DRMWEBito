...
namespace Spartane.WebApi.Controllers
{
    ...
    public partial class Subgrupos_IngredientesController : BaseApiController
    {
        #region Variables
        private ISubgrupos_IngredientesService service = null;
        public  static string ApiControllerUrl { get; set; }
		private ISpartan_Bitacora_SQLService serviceBitacora = null;
		private static int object_id = 44721;
        public string baseApi;
        #endregion Variables

        #region Constructor
        //TODO: Global Data y Data Rerence tienen que estar en un controlador base
        public Subgrupos_IngredientesController(ISubgrupos_IngredientesService service,ISpartan_Bitacora_SQLService bitacoraService)
        {
        ...
        [Authorize]
        [HttpPut]
        public HttpResponseMessage Put(int id, Subgrupos_Ingredientes varSubgrupos_Ingredientes)
        {
            if (ModelState.IsValid && id == varSubgrupos_Ingredientes.Folio)
            {
                var data = "-1";
                try
                {
                    data = Convert.ToString(this.service.Update(varSubgrupos_Ingredientes));
					 var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdSubgrupos_Ingredientes", new JavaScriptSerializer().Serialize(varSubgrupos_Ingredientes), true);
                    serviceBitacora.Insert(bitacora);
                }
                catch (ServiceException ex)
                {
				    var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdSubgrupos_Ingredientes", new JavaScriptSerializer().Serialize(varSubgrupos_Ingredientes), false, ex.Message);
                    serviceBitacora.Insert(bitacora);
                    return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
                }
            ...
            {
               var errors= ModelState.SelectMany(m => m.Value.Errors.Select(err => err.ErrorMessage != string.Empty ? err.ErrorMessage : err.Exception.Message).ToList()).ToList();                
               var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdSubgrupos_Ingredientes", new JavaScriptSerializer().Serialize(varSubgrupos_Ingredientes), false, String.Join(",", errors.ToArray()));
               serviceBitacora.Insert(bitacora);
               return Request.CreateResponse(HttpStatusCode.BadRequest, errors);
            }
        }
...
result.Clasificacion_Clasificacion_Ingredientes = entity.Clasificacion_Clasificacion_Ingredientes;
	...
	[Authorize]
        ...
        public HttpResponseMessage GetAllTunnel(string user, string password)
        {
                ...
                var Subgrupos_IngredientesDataTable = new JavaScriptSerializer().Deserialize<List<Subgrupos_Ingredientes>>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Subgrupos_IngredientesDataTable, Configuration.Formatters.JsonFormatter);
        }
        ...
        public HttpResponseMessage GetTunnel(int id,string user, string password)
        {
                ...
                var Subgrupos_IngredientesResult = new JavaScriptSerializer().Deserialize<Subgrupos_Ingredientes>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Subgrupos_IngredientesResult, Configuration.Formatters.JsonFormatter);            
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage PostTunnel(Subgrupos_Ingredientes emp,string user, string password)
        {
                var client = new System.Net.WebClient();
                client.Headers = TokenManager.GetAuthenticationHeader(user,password);

...
namespace Spartane.WebApi.Controllers
{
    ...
    public partial class Planes_AlimenticiosController : BaseApiController
    {
        #region Variables
        private IPlanes_AlimenticiosService service = null;
        public  static string ApiControllerUrl { get; set; }
		private ISpartan_Bitacora_SQLService serviceBitacora = null;
		private static int object_id = 44568;
        public string baseApi;
        #endregion Variables

        #region Constructor
        //TODO: Global Data y Data Rerence tienen que estar en un controlador base
        public Planes_AlimenticiosController(IPlanes_AlimenticiosService service,ISpartan_Bitacora_SQLService bitacoraService)
        {
        ...
        [Authorize]
        [HttpPut]
        public HttpResponseMessage Put(int id, Planes_Alimenticios varPlanes_Alimenticios)
        {
            if (ModelState.IsValid && id == varPlanes_Alimenticios.Folio)
            {
                var data = "-1";
                try
                {
                    data = Convert.ToString(this.service.Update(varPlanes_Alimenticios));
					 var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdPlanes_Alimenticios", new JavaScriptSerializer().Serialize(varPlanes_Alimenticios), true);
                    serviceBitacora.Insert(bitacora);
                }
                catch (ServiceException ex)
                {
				    var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdPlanes_Alimenticios", new JavaScriptSerializer().Serialize(varPlanes_Alimenticios), false, ex.Message);
                    serviceBitacora.Insert(bitacora);
                    return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
                }
            ...
            {
               var errors= ModelState.SelectMany(m => m.Value.Errors.Select(err => err.ErrorMessage != string.Empty ? err.ErrorMessage : err.Exception.Message).ToList()).ToList();                
               var bitacora = BitacoraHelper.GetBitacora(Request, object_id, id, BitacoraHelper.TypeSql.UPDATE, "sp_UpdPlanes_Alimenticios", new JavaScriptSerializer().Serialize(varPlanes_Alimenticios), false, String.Join(",", errors.ToArray()));
               serviceBitacora.Insert(bitacora);
               return Request.CreateResponse(HttpStatusCode.BadRequest, errors);
            }
        }
...
result.Estatus_Estatus = entity.Estatus_Estatus;
	...
	[Authorize]
        ...
        public HttpResponseMessage GetAllTunnel(string user, string password)
        {
                ...
                var Planes_AlimenticiosDataTable = new JavaScriptSerializer().Deserialize<List<Planes_Alimenticios>>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Planes_AlimenticiosDataTable, Configuration.Formatters.JsonFormatter);
        }
        ...
        public HttpResponseMessage GetTunnel(int id,string user, string password)
        {
                ...
                var Planes_AlimenticiosResult = new JavaScriptSerializer().Deserialize<Planes_Alimenticios>(result);
                return Request.CreateResponse(HttpStatusCode.OK, Planes_AlimenticiosResult, Configuration.Formatters.JsonFormatter);            
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage PostTunnel(Planes_Alimenticios emp,string user, string password)
        {
                var client = new System.Net.WebClient();
                client.Headers = TokenManager.GetAuthenticationHeader(user,password);

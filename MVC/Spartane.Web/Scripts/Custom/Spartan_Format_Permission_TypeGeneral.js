...
function ClearControls() {
    ...
    $('#CreateSpartan_Format_Permission_Type')[0].reset();
    ClearFormControls();
...
function ClearFormControls() {
    $('#CreateSpartan_Format_Permission_Type').trigger('reset');
    $('#CreateSpartan_Format_Permission_Type').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Format_Permission_Type');
...
$(document).ready(function () {
    $("form#CreateSpartan_Format_Permission_Type").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Format_Permission_Type").on('click', '#Spartan_Format_Permission_TypeCancelar', function () {
        BackToGrid();
    });
	$("form#CreateSpartan_Format_Permission_Type").on('click', '#Spartan_Format_Permission_TypeGuardar', function () {
        EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Format_Permission_TypeData(function () {
                EjecutarValidacionesDespuesDeGuardar();
                BackToGrid();
            });
    });
	$("form#CreateSpartan_Format_Permission_Type").on('click', '#Spartan_Format_Permission_TypeGuardarYNuevo', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation()) {
            SendSpartan_Format_Permission_TypeData(function () {
                ClearControls();
    ...
    $("form#CreateSpartan_Format_Permission_Type").on('click', '#Spartan_Format_Permission_TypeGuardarYCopia', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Format_Permission_TypeData(function (currentId) {
                $("#PermissionTypeIdId").val("0");
...
var Spartan_Format_Permission_TypeisdisplayBusinessPropery = false;
Spartan_Format_Permission_TypeDisplayBusinessRuleButtons(Spartan_Format_Permission_TypeisdisplayBusinessPropery);
function Spartan_Format_Permission_TypeDisplayBusinessRule() {
    if (!Spartan_Format_Permission_TypeisdisplayBusinessPropery) {
        $('#CreateSpartan_Format_Permission_Type').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_Format_Permission_TypedisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Format_Permission_TypePropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Format_Permission_TypedisplayBusinessPropery').remove();
    }
    Spartan_Format_Permission_TypeDisplayBusinessRuleButtons(!Spartan_Format_Permission_TypeisdisplayBusinessPropery);
    Spartan_Format_Permission_TypeisdisplayBusinessPropery = (Spartan_Format_Permission_TypeisdisplayBusinessPropery ? false : true);
}
function Spartan_Format_Permission_TypeDisplayBusinessRuleButtons(flag) {

...
function ClearControls() {
    $("#ReferenceParameterTypeId").val("0");
    $('#CreateSpartan_BR_Action_Param_Type')[0].reset();
    ClearFormControls();
    $("#ParameterTypeIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Action_Param_Type').trigger('reset');
    $('#CreateSpartan_BR_Action_Param_Type').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Action_Param_Type');
...
function ResetClaveLabel() {
    $("#lblParameterTypeId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Action_Param_Type").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Action_Param_TypeData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Action_Param_TypeData(function (currentId) {
                $("#ParameterTypeIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceParameterTypeId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Action_Param_Type').find('.col-sm-8').each(function () {

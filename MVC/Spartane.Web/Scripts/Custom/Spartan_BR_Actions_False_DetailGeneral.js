...
function ClearControls() {
    $("#ReferenceActionsFalseDetailId").val("0");
    $('#CreateSpartan_BR_Actions_False_Detail')[0].reset();
    ClearFormControls();
    $("#ActionsFalseDetailIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Actions_False_Detail').trigger('reset');
    $('#CreateSpartan_BR_Actions_False_Detail').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Actions_False_Detail');
...
function ResetClaveLabel() {
    $("#lblActionsFalseDetailId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Actions_False_Detail").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Actions_False_DetailData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Actions_False_DetailData(function (currentId) {
                $("#ActionsFalseDetailIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceActionsFalseDetailId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Actions_False_Detail').find('.col-sm-8').each(function () {

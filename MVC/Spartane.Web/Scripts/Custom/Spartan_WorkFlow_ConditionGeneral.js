...
function ClearControls() {
    $("#ReferenceConditionId").val("0");
    $('#CreateSpartan_WorkFlow_Condition')[0].reset();
    ClearFormControls();
    $("#ConditionIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_WorkFlow_Condition').trigger('reset');
    $('#CreateSpartan_WorkFlow_Condition').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_WorkFlow_Condition');
...
function ResetClaveLabel() {
    $("#lblConditionId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_WorkFlow_Condition").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_WorkFlow_Condition").on('click', '#Spartan_WorkFlow_ConditionCancelar', function () {
        Spartan_WorkFlow_ConditionBackToGrid();
    });
	$("form#CreateSpartan_WorkFlow_Condition").on('click', '#Spartan_WorkFlow_ConditionGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_ConditionData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_WorkFlow_ConditionBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_WorkFlow_Condition").on('click', '#Spartan_WorkFlow_ConditionGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_WorkFlow_ConditionData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_WorkFlow_Condition").on('click', '#Spartan_WorkFlow_ConditionGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_ConditionData(function (currentId) {
					$("#ConditionIdId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceConditionId").val(currentId);
...
var Spartan_WorkFlow_ConditionisdisplayBusinessPropery = false;
Spartan_WorkFlow_ConditionDisplayBusinessRuleButtons(Spartan_WorkFlow_ConditionisdisplayBusinessPropery);
function Spartan_WorkFlow_ConditionDisplayBusinessRule() {
    if (!Spartan_WorkFlow_ConditionisdisplayBusinessPropery) {
        $('#CreateSpartan_WorkFlow_Condition').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_WorkFlow_ConditiondisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_WorkFlow_ConditionPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_WorkFlow_ConditiondisplayBusinessPropery').remove();
    }
    Spartan_WorkFlow_ConditionDisplayBusinessRuleButtons(!Spartan_WorkFlow_ConditionisdisplayBusinessPropery);
    Spartan_WorkFlow_ConditionisdisplayBusinessPropery = (Spartan_WorkFlow_ConditionisdisplayBusinessPropery ? false : true);
}
function Spartan_WorkFlow_ConditionDisplayBusinessRuleButtons(flag) {

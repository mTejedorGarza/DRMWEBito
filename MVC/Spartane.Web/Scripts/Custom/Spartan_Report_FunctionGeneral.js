...
function ClearControls() {
    $("#ReferenceFunctionId").val("0");
    $('#CreateSpartan_Report_Function')[0].reset();
    ClearFormControls();
    $("#FunctionIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Report_Function').trigger('reset');
    $('#CreateSpartan_Report_Function').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Report_Function');
...
function ResetClaveLabel() {
    $("#lblFunctionId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_Report_Function").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Report_Function").on('click', '#Spartan_Report_FunctionCancelar', function () {
        BackToGrid();
    });
	$("form#CreateSpartan_Report_Function").on('click', '#Spartan_Report_FunctionGuardar', function () {
        EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Report_FunctionData(function () {
                EjecutarValidacionesDespuesDeGuardar();
                BackToGrid();
            });
    });
	$("form#CreateSpartan_Report_Function").on('click', '#Spartan_Report_FunctionGuardarYNuevo', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation()) {
            SendSpartan_Report_FunctionData(function () {
                ClearControls();
    ...
    $("form#CreateSpartan_Report_Function").on('click', '#Spartan_Report_FunctionGuardarYCopia', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Report_FunctionData(function (currentId) {
                $("#FunctionIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceFunctionId").val(currentId);
...
var Spartan_Report_FunctionisdisplayBusinessPropery = false;
Spartan_Report_FunctionDisplayBusinessRuleButtons(Spartan_Report_FunctionisdisplayBusinessPropery);
function Spartan_Report_FunctionDisplayBusinessRule() {
    if (!Spartan_Report_FunctionisdisplayBusinessPropery) {
        $('#CreateSpartan_Report_Function').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_Report_FunctiondisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Report_FunctionPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Report_FunctiondisplayBusinessPropery').remove();
    }
    Spartan_Report_FunctionDisplayBusinessRuleButtons(!Spartan_Report_FunctionisdisplayBusinessPropery);
    Spartan_Report_FunctionisdisplayBusinessPropery = (Spartan_Report_FunctionisdisplayBusinessPropery ? false : true);
}
function Spartan_Report_FunctionDisplayBusinessRuleButtons(flag) {

...
function ClearControls() {
    $("#ReferenceOperationDetailId").val("0");
    $('#CreateSpartan_BR_Operation_Detail')[0].reset();
    ClearFormControls();
    $("#OperationDetailIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Operation_Detail').trigger('reset');
    $('#CreateSpartan_BR_Operation_Detail').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Operation_Detail');
...
function ResetClaveLabel() {
    $("#lblOperationDetailId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Operation_Detail").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Operation_DetailData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Operation_DetailData(function (currentId) {
                $("#OperationDetailIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceOperationDetailId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Operation_Detail').find('.col-sm-8').each(function () {

...
function ClearControls() {
    ...
    $('#CreateSpartan_BR_Status')[0].reset();
    ClearFormControls();
    $("#StatusIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Status').trigger('reset');
    $('#CreateSpartan_BR_Status').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Status');
...
$(document).ready(function () {
    $("form#CreateSpartan_BR_Status").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_StatusData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_StatusData(function (currentId) {
                $("#StatusIdId").val("0");
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Status').find('.col-sm-8').each(function () {

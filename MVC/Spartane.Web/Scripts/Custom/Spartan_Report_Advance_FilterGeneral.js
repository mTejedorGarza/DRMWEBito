...
function ClearControls() {
    ...
    $('#CreateSpartan_Report_Advance_Filter')[0].reset();
    ClearFormControls();
    $("#ClaveId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Report_Advance_Filter').trigger('reset');
    $('#CreateSpartan_Report_Advance_Filter').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Report_Advance_Filter');
...
$(document).ready(function () {
    $("form#CreateSpartan_Report_Advance_Filter").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Report_Advance_Filter").on('click', '#Spartan_Report_Advance_FilterCancelar', function () {
        Spartan_Report_Advance_FilterBackToGrid();
    });
	$("form#CreateSpartan_Report_Advance_Filter").on('click', '#Spartan_Report_Advance_FilterGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Report_Advance_FilterData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_Report_Advance_FilterBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_Report_Advance_Filter").on('click', '#Spartan_Report_Advance_FilterGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_Report_Advance_FilterData(function () {
				EjecutarValidacionesDespuesDeGuardar();		
    ...
    $("form#CreateSpartan_Report_Advance_Filter").on('click', '#Spartan_Report_Advance_FilterGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Report_Advance_FilterData(function (currentId) {
					$("#ClaveId").val("0");
...
var Spartan_Report_Advance_FilterisdisplayBusinessPropery = false;
Spartan_Report_Advance_FilterDisplayBusinessRuleButtons(Spartan_Report_Advance_FilterisdisplayBusinessPropery);
function Spartan_Report_Advance_FilterDisplayBusinessRule() {
    if (!Spartan_Report_Advance_FilterisdisplayBusinessPropery) {
        $('#CreateSpartan_Report_Advance_Filter').find('.col-sm-8').each(function () {
			var div =$(this); 
            ...
            mainElementAttributes += '<div class="Spartan_Report_Advance_FilterdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Report_Advance_FilterPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Report_Advance_FilterdisplayBusinessPropery').remove();
    }
    Spartan_Report_Advance_FilterDisplayBusinessRuleButtons(!Spartan_Report_Advance_FilterisdisplayBusinessPropery);
    Spartan_Report_Advance_FilterisdisplayBusinessPropery = (Spartan_Report_Advance_FilterisdisplayBusinessPropery ? false : true);
}
function Spartan_Report_Advance_FilterDisplayBusinessRuleButtons(flag) {

...
function ClearControls() {
    $("#ReferenceIdConcept").val("0");
    $('#CreateSpartan_Traduction_Concept_Type')[0].reset();
    ClearFormControls();
    $("#IdConceptId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Traduction_Concept_Type').trigger('reset');
    $('#CreateSpartan_Traduction_Concept_Type').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Traduction_Concept_Type');
...
$(document).ready(function () {
    $("form#CreateSpartan_Traduction_Concept_Type").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Traduction_Concept_Type").on('click', '#Spartan_Traduction_Concept_TypeCancelar', function () {
        Spartan_Traduction_Concept_TypeBackToGrid();
    });
	$("form#CreateSpartan_Traduction_Concept_Type").on('click', '#Spartan_Traduction_Concept_TypeGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Traduction_Concept_TypeData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_Traduction_Concept_TypeBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_Traduction_Concept_Type").on('click', '#Spartan_Traduction_Concept_TypeGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_Traduction_Concept_TypeData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_Traduction_Concept_Type").on('click', '#Spartan_Traduction_Concept_TypeGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Traduction_Concept_TypeData(function (currentId) {
					$("#IdConceptId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceIdConcept").val(currentId);
...
var Spartan_Traduction_Concept_TypeisdisplayBusinessPropery = false;
Spartan_Traduction_Concept_TypeDisplayBusinessRuleButtons(Spartan_Traduction_Concept_TypeisdisplayBusinessPropery);
function Spartan_Traduction_Concept_TypeDisplayBusinessRule() {
    if (!Spartan_Traduction_Concept_TypeisdisplayBusinessPropery) {
        $('#CreateSpartan_Traduction_Concept_Type').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_Traduction_Concept_TypedisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Traduction_Concept_TypePropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Traduction_Concept_TypedisplayBusinessPropery').remove();
    }
    Spartan_Traduction_Concept_TypeDisplayBusinessRuleButtons(!Spartan_Traduction_Concept_TypeisdisplayBusinessPropery);
    Spartan_Traduction_Concept_TypeisdisplayBusinessPropery = (Spartan_Traduction_Concept_TypeisdisplayBusinessPropery ? false : true);
}
function Spartan_Traduction_Concept_TypeDisplayBusinessRuleButtons(flag) {

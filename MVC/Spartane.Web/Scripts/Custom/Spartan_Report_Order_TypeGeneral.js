...
function ClearControls() {
    $("#ReferenceOrderTypeId").val("0");
    $('#CreateSpartan_Report_Order_Type')[0].reset();
    ClearFormControls();
    $("#OrderTypeIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Report_Order_Type').trigger('reset');
    $('#CreateSpartan_Report_Order_Type').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Report_Order_Type');
...
$(document).ready(function () {
    $("form#CreateSpartan_Report_Order_Type").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Report_Order_Type").on('click', '#Spartan_Report_Order_TypeCancelar', function () {
        BackToGrid();
    });
	$("form#CreateSpartan_Report_Order_Type").on('click', '#Spartan_Report_Order_TypeGuardar', function () {
        EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Report_Order_TypeData(function () {
                EjecutarValidacionesDespuesDeGuardar();
                BackToGrid();
            });
    });
	$("form#CreateSpartan_Report_Order_Type").on('click', '#Spartan_Report_Order_TypeGuardarYNuevo', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation()) {
            SendSpartan_Report_Order_TypeData(function () {
                ClearControls();
    ...
    $("form#CreateSpartan_Report_Order_Type").on('click', '#Spartan_Report_Order_TypeGuardarYCopia', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Report_Order_TypeData(function (currentId) {
                $("#OrderTypeIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceOrderTypeId").val(currentId);
...
var Spartan_Report_Order_TypeisdisplayBusinessPropery = false;
Spartan_Report_Order_TypeDisplayBusinessRuleButtons(Spartan_Report_Order_TypeisdisplayBusinessPropery);
function Spartan_Report_Order_TypeDisplayBusinessRule() {
    if (!Spartan_Report_Order_TypeisdisplayBusinessPropery) {
        $('#CreateSpartan_Report_Order_Type').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_Report_Order_TypedisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Report_Order_TypePropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Report_Order_TypedisplayBusinessPropery').remove();
    }
    Spartan_Report_Order_TypeDisplayBusinessRuleButtons(!Spartan_Report_Order_TypeisdisplayBusinessPropery);
    Spartan_Report_Order_TypeisdisplayBusinessPropery = (Spartan_Report_Order_TypeisdisplayBusinessPropery ? false : true);
}
function Spartan_Report_Order_TypeDisplayBusinessRuleButtons(flag) {

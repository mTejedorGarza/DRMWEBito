...
function ClearControls() {
    $("#ReferenceKey_History").val("0");
    $('#CreateSpartan_BR_History')[0].reset();
    ClearFormControls();
    $("#Key_HistoryId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_History').trigger('reset');
    $('#CreateSpartan_BR_History').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_History');
...
function ResetClaveLabel() {
    $("#lblKey_History").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_History").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_BR_History").on('click', '#Spartan_BR_HistoryCancelar', function () {
        Spartan_BR_HistoryBackToGrid();
    });
	$("form#CreateSpartan_BR_History").on('click', '#Spartan_BR_HistoryGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_BR_HistoryData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_BR_HistoryBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_BR_History").on('click', '#Spartan_BR_HistoryGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_BR_HistoryData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_BR_History").on('click', '#Spartan_BR_HistoryGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_BR_HistoryData(function (currentId) {
					$("#Key_HistoryId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceKey_History").val(currentId);
...
var Spartan_BR_HistoryisdisplayBusinessPropery = false;
Spartan_BR_HistoryDisplayBusinessRuleButtons(Spartan_BR_HistoryisdisplayBusinessPropery);
function Spartan_BR_HistoryDisplayBusinessRule() {
    if (!Spartan_BR_HistoryisdisplayBusinessPropery) {
        $('#CreateSpartan_BR_History').find('.col-sm-8').each(function () {
			var div =$(this); 
            ...
            mainElementAttributes += '<div class="Spartan_BR_HistorydisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_BR_HistoryPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_BR_HistorydisplayBusinessPropery').remove();
    }
    Spartan_BR_HistoryDisplayBusinessRuleButtons(!Spartan_BR_HistoryisdisplayBusinessPropery);
    Spartan_BR_HistoryisdisplayBusinessPropery = (Spartan_BR_HistoryisdisplayBusinessPropery ? false : true);
}
function Spartan_BR_HistoryDisplayBusinessRuleButtons(flag) {

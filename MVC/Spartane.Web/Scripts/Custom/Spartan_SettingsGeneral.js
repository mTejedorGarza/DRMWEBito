...
function ClearControls() {
    ...
    $('#CreateSpartan_Settings')[0].reset();
    ClearFormControls();
    $("#ClaveId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Settings').trigger('reset');
    $('#CreateSpartan_Settings').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Settings');
...
$(document).ready(function () {
    $("form#CreateSpartan_Settings").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Settings").on('click', '#Spartan_SettingsCancelar', function () {
        Spartan_SettingsBackToGrid();
    });
	$("form#CreateSpartan_Settings").on('click', '#Spartan_SettingsGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_SettingsData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_SettingsBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_Settings").on('click', '#Spartan_SettingsGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_SettingsData(function () {
				EjecutarValidacionesDespuesDeGuardar();		
    ...
    $("form#CreateSpartan_Settings").on('click', '#Spartan_SettingsGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_SettingsData(function (currentId) {
					$("#ClaveId").val("0");
...
var Spartan_SettingsisdisplayBusinessPropery = false;
Spartan_SettingsDisplayBusinessRuleButtons(Spartan_SettingsisdisplayBusinessPropery);
function Spartan_SettingsDisplayBusinessRule() {
    if (!Spartan_SettingsisdisplayBusinessPropery) {
        $('#CreateSpartan_Settings').find('.col-sm-8').each(function () {
			var div =$(this); 
            ...
            mainElementAttributes += '<div class="Spartan_SettingsdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_SettingsPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_SettingsdisplayBusinessPropery').remove();
    }
    Spartan_SettingsDisplayBusinessRuleButtons(!Spartan_SettingsisdisplayBusinessPropery);
    Spartan_SettingsisdisplayBusinessPropery = (Spartan_SettingsisdisplayBusinessPropery ? false : true);
}
function Spartan_SettingsDisplayBusinessRuleButtons(flag) {

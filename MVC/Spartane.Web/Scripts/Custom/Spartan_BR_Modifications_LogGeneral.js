...
function ClearControls() {
    $("#ReferenceModificationId").val("0");
    $('#CreateSpartan_BR_Modifications_Log')[0].reset();
    ClearFormControls();
    $("#ModificationIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Modifications_Log').trigger('reset');
    $('#CreateSpartan_BR_Modifications_Log').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Modifications_Log');
...
function ResetClaveLabel() {
    $("#lblModificationId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Modifications_Log").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Modifications_LogData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Modifications_LogData(function (currentId) {
                $("#ModificationIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceModificationId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Modifications_Log').find('.col-sm-8').each(function () {

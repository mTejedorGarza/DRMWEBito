...
function ClearControls() {
    $("#ReferenceAttributeId").val("0");
    $('#CreateSpartan_Metadata')[0].reset();
    ClearFormControls();
    $("#AttributeIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Metadata').trigger('reset');
    $('#CreateSpartan_Metadata').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Metadata');
...
function ResetClaveLabel() {
    $("#lblAttributeId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_Metadata").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Metadata").on('click', '#Spartan_MetadataCancelar', function () {
        BackToGrid();
    });
	$("form#CreateSpartan_Metadata").on('click', '#Spartan_MetadataGuardar', function () {
        EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_MetadataData(function () {
                EjecutarValidacionesDespuesDeGuardar();
                BackToGrid();
            });
    });
	$("form#CreateSpartan_Metadata").on('click', '#Spartan_MetadataGuardarYNuevo', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation()) {
            SendSpartan_MetadataData(function () {
                ClearControls();
    ...
    $("form#CreateSpartan_Metadata").on('click', '#Spartan_MetadataGuardarYCopia', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_MetadataData(function (currentId) {
                $("#AttributeIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceAttributeId").val(currentId);
...
var Spartan_MetadataisdisplayBusinessPropery = false;
Spartan_MetadataDisplayBusinessRuleButtons(Spartan_MetadataisdisplayBusinessPropery);
function Spartan_MetadataDisplayBusinessRule() {
    if (!Spartan_MetadataisdisplayBusinessPropery) {
        $('#CreateSpartan_Metadata').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="displayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_MetadataPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_MetadatadisplayBusinessPropery').remove();
    }
    Spartan_MetadataDisplayBusinessRuleButtons(!Spartan_MetadataisdisplayBusinessPropery);
    Spartan_MetadataisdisplayBusinessPropery = (Spartan_MetadataisdisplayBusinessPropery ? false : true);
}
function Spartan_MetadataDisplayBusinessRuleButtons(flag) {

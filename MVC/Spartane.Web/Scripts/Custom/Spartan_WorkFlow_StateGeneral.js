...
function ClearControls() {
    $("#ReferenceStateId").val("0");
    $('#CreateSpartan_WorkFlow_State')[0].reset();
    ClearFormControls();
    $("#StateIdId").val("0");
    $('#Attribute').empty();
...
function ClearFormControls() {
    $('#CreateSpartan_WorkFlow_State').trigger('reset');
    $('#CreateSpartan_WorkFlow_State').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_WorkFlow_State');
...
$(document).ready(function () {
    $("form#CreateSpartan_WorkFlow_State").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_WorkFlow_State").on('click', '#Spartan_WorkFlow_StateCancelar', function () {
        Spartan_WorkFlow_StateBackToGrid();
    });
	$("form#CreateSpartan_WorkFlow_State").on('click', '#Spartan_WorkFlow_StateGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_StateData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_WorkFlow_StateBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_WorkFlow_State").on('click', '#Spartan_WorkFlow_StateGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_WorkFlow_StateData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_WorkFlow_State").on('click', '#Spartan_WorkFlow_StateGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_StateData(function (currentId) {
					$("#StateIdId").val("0");
	    $('#Attribute').empty();
    $("#Attribute").append('<option value=""></option>');
    $('#Attribute').val('0').trigger('change');
					...
					ResetClaveLabel();
					$("#ReferenceStateId").val(currentId);
...
var Spartan_WorkFlow_StateisdisplayBusinessPropery = false;
Spartan_WorkFlow_StateDisplayBusinessRuleButtons(Spartan_WorkFlow_StateisdisplayBusinessPropery);
function Spartan_WorkFlow_StateDisplayBusinessRule() {
    if (!Spartan_WorkFlow_StateisdisplayBusinessPropery) {
        $('#CreateSpartan_WorkFlow_State').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_WorkFlow_StatedisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_WorkFlow_StatePropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_WorkFlow_StatedisplayBusinessPropery').remove();
    }
    Spartan_WorkFlow_StateDisplayBusinessRuleButtons(!Spartan_WorkFlow_StateisdisplayBusinessPropery);
    Spartan_WorkFlow_StateisdisplayBusinessPropery = (Spartan_WorkFlow_StateisdisplayBusinessPropery ? false : true);
}
function Spartan_WorkFlow_StateDisplayBusinessRuleButtons(flag) {

...
function ClearControls() {
    $("#ReferenceType_of_Work_DistributionId").val("0");
    $('#CreateSpartan_WorkFlow_Type_Work_Distribution')[0].reset();
    ClearFormControls();
    $("#Type_of_Work_DistributionIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_WorkFlow_Type_Work_Distribution').trigger('reset');
    $('#CreateSpartan_WorkFlow_Type_Work_Distribution').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_WorkFlow_Type_Work_Distribution');
...
function ResetClaveLabel() {
    $("#lblType_of_Work_DistributionId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_WorkFlow_Type_Work_Distribution").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_WorkFlow_Type_Work_Distribution").on('click', '#Spartan_WorkFlow_Type_Work_DistributionCancelar', function () {
        Spartan_WorkFlow_Type_Work_DistributionBackToGrid();
    });
	$("form#CreateSpartan_WorkFlow_Type_Work_Distribution").on('click', '#Spartan_WorkFlow_Type_Work_DistributionGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_Type_Work_DistributionData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_WorkFlow_Type_Work_DistributionBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_WorkFlow_Type_Work_Distribution").on('click', '#Spartan_WorkFlow_Type_Work_DistributionGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_WorkFlow_Type_Work_DistributionData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_WorkFlow_Type_Work_Distribution").on('click', '#Spartan_WorkFlow_Type_Work_DistributionGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_Type_Work_DistributionData(function (currentId) {
					$("#Type_of_Work_DistributionIdId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceType_of_Work_DistributionId").val(currentId);
...
var Spartan_WorkFlow_Type_Work_DistributionisdisplayBusinessPropery = false;
Spartan_WorkFlow_Type_Work_DistributionDisplayBusinessRuleButtons(Spartan_WorkFlow_Type_Work_DistributionisdisplayBusinessPropery);
function Spartan_WorkFlow_Type_Work_DistributionDisplayBusinessRule() {
    if (!Spartan_WorkFlow_Type_Work_DistributionisdisplayBusinessPropery) {
        $('#CreateSpartan_WorkFlow_Type_Work_Distribution').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_WorkFlow_Type_Work_DistributiondisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_WorkFlow_Type_Work_DistributionPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_WorkFlow_Type_Work_DistributiondisplayBusinessPropery').remove();
    }
    Spartan_WorkFlow_Type_Work_DistributionDisplayBusinessRuleButtons(!Spartan_WorkFlow_Type_Work_DistributionisdisplayBusinessPropery);
    Spartan_WorkFlow_Type_Work_DistributionisdisplayBusinessPropery = (Spartan_WorkFlow_Type_Work_DistributionisdisplayBusinessPropery ? false : true);
}
function Spartan_WorkFlow_Type_Work_DistributionDisplayBusinessRuleButtons(flag) {

...
function ClearControls() {
    $("#ReferenceKey_Testing").val("0");
    $('#CreateSpartan_BR_Testing')[0].reset();
    ClearFormControls();
    $("#Key_TestingId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Testing').trigger('reset');
    $('#CreateSpartan_BR_Testing').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Testing');
...
function ResetClaveLabel() {
    $("#lblKey_Testing").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Testing").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_BR_Testing").on('click', '#Spartan_BR_TestingCancelar', function () {
        Spartan_BR_TestingBackToGrid();
    });
	$("form#CreateSpartan_BR_Testing").on('click', '#Spartan_BR_TestingGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_BR_TestingData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_BR_TestingBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_BR_Testing").on('click', '#Spartan_BR_TestingGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_BR_TestingData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_BR_Testing").on('click', '#Spartan_BR_TestingGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_BR_TestingData(function (currentId) {
					$("#Key_TestingId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceKey_Testing").val(currentId);
...
var Spartan_BR_TestingisdisplayBusinessPropery = false;
Spartan_BR_TestingDisplayBusinessRuleButtons(Spartan_BR_TestingisdisplayBusinessPropery);
function Spartan_BR_TestingDisplayBusinessRule() {
    if (!Spartan_BR_TestingisdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Testing').find('.col-sm-8').each(function () {
			var div =$(this); 
            ...
            mainElementAttributes += '<div class="Spartan_BR_TestingdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_BR_TestingPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_BR_TestingdisplayBusinessPropery').remove();
    }
    Spartan_BR_TestingDisplayBusinessRuleButtons(!Spartan_BR_TestingisdisplayBusinessPropery);
    Spartan_BR_TestingisdisplayBusinessPropery = (Spartan_BR_TestingisdisplayBusinessPropery ? false : true);
}
function Spartan_BR_TestingDisplayBusinessRuleButtons(flag) {

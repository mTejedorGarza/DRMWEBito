...
function ClearControls() {
    $("#ReferenceKey_Peer_Review").val("0");
    $('#CreateSpartan_BR_Peer_Review')[0].reset();
    ClearFormControls();
    $("#Key_Peer_ReviewId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Peer_Review').trigger('reset');
    $('#CreateSpartan_BR_Peer_Review').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Peer_Review');
...
function ResetClaveLabel() {
    $("#lblKey_Peer_Review").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Peer_Review").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_BR_Peer_Review").on('click', '#Spartan_BR_Peer_ReviewCancelar', function () {
        Spartan_BR_Peer_ReviewBackToGrid();
    });
	$("form#CreateSpartan_BR_Peer_Review").on('click', '#Spartan_BR_Peer_ReviewGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_BR_Peer_ReviewData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_BR_Peer_ReviewBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_BR_Peer_Review").on('click', '#Spartan_BR_Peer_ReviewGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_BR_Peer_ReviewData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_BR_Peer_Review").on('click', '#Spartan_BR_Peer_ReviewGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_BR_Peer_ReviewData(function (currentId) {
					$("#Key_Peer_ReviewId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceKey_Peer_Review").val(currentId);
...
var Spartan_BR_Peer_ReviewisdisplayBusinessPropery = false;
Spartan_BR_Peer_ReviewDisplayBusinessRuleButtons(Spartan_BR_Peer_ReviewisdisplayBusinessPropery);
function Spartan_BR_Peer_ReviewDisplayBusinessRule() {
    if (!Spartan_BR_Peer_ReviewisdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Peer_Review').find('.col-sm-8').each(function () {
			var div =$(this); 
            ...
            mainElementAttributes += '<div class="Spartan_BR_Peer_ReviewdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_BR_Peer_ReviewPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_BR_Peer_ReviewdisplayBusinessPropery').remove();
    }
    Spartan_BR_Peer_ReviewDisplayBusinessRuleButtons(!Spartan_BR_Peer_ReviewisdisplayBusinessPropery);
    Spartan_BR_Peer_ReviewisdisplayBusinessPropery = (Spartan_BR_Peer_ReviewisdisplayBusinessPropery ? false : true);
}
function Spartan_BR_Peer_ReviewDisplayBusinessRuleButtons(flag) {

...
function ClearControls() {
    $("#ReferenceDesignDetailId").val("0");
    $('#CreateSpartan_Report_Fields_Detail')[0].reset();
    ClearFormControls();
    $("#DesignDetailIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Report_Fields_Detail').trigger('reset');
    $('#CreateSpartan_Report_Fields_Detail').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Report_Fields_Detail');
...
function ResetClaveLabel() {
    $("#lblDesignDetailId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_Report_Fields_Detail").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Report_Fields_Detail").on('click', '#Spartan_Report_Fields_DetailCancelar', function () {
        Spartan_Report_Fields_DetailBackToGrid();
    });
	$("form#CreateSpartan_Report_Fields_Detail").on('click', '#Spartan_Report_Fields_DetailGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Report_Fields_DetailData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_Report_Fields_DetailBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_Report_Fields_Detail").on('click', '#Spartan_Report_Fields_DetailGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_Report_Fields_DetailData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_Report_Fields_Detail").on('click', '#Spartan_Report_Fields_DetailGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Report_Fields_DetailData(function (currentId) {
					$("#DesignDetailIdId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceDesignDetailId").val(currentId);
...
var Spartan_Report_Fields_DetailisdisplayBusinessPropery = false;
Spartan_Report_Fields_DetailDisplayBusinessRuleButtons(Spartan_Report_Fields_DetailisdisplayBusinessPropery);
function Spartan_Report_Fields_DetailDisplayBusinessRule() {
    if (!Spartan_Report_Fields_DetailisdisplayBusinessPropery) {
        $('#CreateSpartan_Report_Fields_Detail').find('.col-sm-8').each(function () {
			var div =$(this); 
            ...
            mainElementAttributes += '<div class="Spartan_Report_Fields_DetaildisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Report_Fields_DetailPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Report_Fields_DetaildisplayBusinessPropery').remove();
    }
    Spartan_Report_Fields_DetailDisplayBusinessRuleButtons(!Spartan_Report_Fields_DetailisdisplayBusinessPropery);
    Spartan_Report_Fields_DetailisdisplayBusinessPropery = (Spartan_Report_Fields_DetailisdisplayBusinessPropery ? false : true);
}
function Spartan_Report_Fields_DetailDisplayBusinessRuleButtons(flag) {

...
function ClearControls() {
    $("#ReferenceActionResultId").val("0");
    $('#CreateSpartan_BR_Action_Result')[0].reset();
    ClearFormControls();
    $("#ActionResultIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Action_Result').trigger('reset');
    $('#CreateSpartan_BR_Action_Result').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Action_Result');
...
function ResetClaveLabel() {
    $("#lblActionResultId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Action_Result").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Action_ResultData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Action_ResultData(function (currentId) {
                $("#ActionResultIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceActionResultId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Action_Result').find('.col-sm-8').each(function () {

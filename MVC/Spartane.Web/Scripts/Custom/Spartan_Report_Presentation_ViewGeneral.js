...
function ClearControls() {
    $("#ReferencePresentationViewId").val("0");
    $('#CreateSpartan_Report_Presentation_View')[0].reset();
    ClearFormControls();
    $("#PresentationViewIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Report_Presentation_View').trigger('reset');
    $('#CreateSpartan_Report_Presentation_View').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Report_Presentation_View');
...
function ResetClaveLabel() {
    $("#lblPresentationViewId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_Report_Presentation_View").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Report_Presentation_View").on('click', '#Spartan_Report_Presentation_ViewCancelar', function () {
        BackToGrid();
    });
	$("form#CreateSpartan_Report_Presentation_View").on('click', '#Spartan_Report_Presentation_ViewGuardar', function () {
        EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Report_Presentation_ViewData(function () {
                EjecutarValidacionesDespuesDeGuardar();
                BackToGrid();
            });
    });
	$("form#CreateSpartan_Report_Presentation_View").on('click', '#Spartan_Report_Presentation_ViewGuardarYNuevo', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation()) {
            SendSpartan_Report_Presentation_ViewData(function () {
                ClearControls();
    ...
    $("form#CreateSpartan_Report_Presentation_View").on('click', '#Spartan_Report_Presentation_ViewGuardarYCopia', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Report_Presentation_ViewData(function (currentId) {
                $("#PresentationViewIdId").val("0");

                ResetClaveLabel();
                $("#ReferencePresentationViewId").val(currentId);
...
var Spartan_Report_Presentation_ViewisdisplayBusinessPropery = false;
Spartan_Report_Presentation_ViewDisplayBusinessRuleButtons(Spartan_Report_Presentation_ViewisdisplayBusinessPropery);
function Spartan_Report_Presentation_ViewDisplayBusinessRule() {
    if (!Spartan_Report_Presentation_ViewisdisplayBusinessPropery) {
        $('#CreateSpartan_Report_Presentation_View').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_Report_Presentation_ViewdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Report_Presentation_ViewPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Report_Presentation_ViewdisplayBusinessPropery').remove();
    }
    Spartan_Report_Presentation_ViewDisplayBusinessRuleButtons(!Spartan_Report_Presentation_ViewisdisplayBusinessPropery);
    Spartan_Report_Presentation_ViewisdisplayBusinessPropery = (Spartan_Report_Presentation_ViewisdisplayBusinessPropery ? false : true);
}
function Spartan_Report_Presentation_ViewDisplayBusinessRuleButtons(flag) {

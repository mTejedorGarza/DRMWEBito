...
function ClearControls() {
    $("#ReferenceClassificationId").val("0");
    $('#CreateSpartan_BR_Action_Classification')[0].reset();
    ClearFormControls();
    $("#ClassificationIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Action_Classification').trigger('reset');
    $('#CreateSpartan_BR_Action_Classification').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Action_Classification');
...
$(document).ready(function () {
    $("form#CreateSpartan_BR_Action_Classification").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Action_ClassificationData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Action_ClassificationData(function (currentId) {
                $("#ClassificationIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceClassificationId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Action_Classification').find('.col-sm-8').each(function () {

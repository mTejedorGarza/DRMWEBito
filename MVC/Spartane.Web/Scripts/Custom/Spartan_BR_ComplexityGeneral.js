...
function ClearControls() {
    $("#ReferenceKey_Complexity").val("0");
    $('#CreateSpartan_BR_Complexity')[0].reset();
    ClearFormControls();
    $("#Key_ComplexityId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Complexity').trigger('reset');
    $('#CreateSpartan_BR_Complexity').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Complexity');
...
function ResetClaveLabel() {
    $("#lblKey_Complexity").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Complexity").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_BR_Complexity").on('click', '#Spartan_BR_ComplexityCancelar', function () {
        Spartan_BR_ComplexityBackToGrid();
    });
	$("form#CreateSpartan_BR_Complexity").on('click', '#Spartan_BR_ComplexityGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_BR_ComplexityData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_BR_ComplexityBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_BR_Complexity").on('click', '#Spartan_BR_ComplexityGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_BR_ComplexityData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_BR_Complexity").on('click', '#Spartan_BR_ComplexityGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_BR_ComplexityData(function (currentId) {
					$("#Key_ComplexityId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceKey_Complexity").val(currentId);
...
var Spartan_BR_ComplexityisdisplayBusinessPropery = false;
Spartan_BR_ComplexityDisplayBusinessRuleButtons(Spartan_BR_ComplexityisdisplayBusinessPropery);
function Spartan_BR_ComplexityDisplayBusinessRule() {
    if (!Spartan_BR_ComplexityisdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Complexity').find('.col-sm-8').each(function () {
			var div =$(this); 
            ...
            mainElementAttributes += '<div class="Spartan_BR_ComplexitydisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_BR_ComplexityPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_BR_ComplexitydisplayBusinessPropery').remove();
    }
    Spartan_BR_ComplexityDisplayBusinessRuleButtons(!Spartan_BR_ComplexityisdisplayBusinessPropery);
    Spartan_BR_ComplexityisdisplayBusinessPropery = (Spartan_BR_ComplexityisdisplayBusinessPropery ? false : true);
}
function Spartan_BR_ComplexityDisplayBusinessRuleButtons(flag) {

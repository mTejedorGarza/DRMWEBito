...
function ClearControls() {
    $("#ReferenceScopeId").val("0");
    $('#CreateSpartan_BR_Scope')[0].reset();
    ClearFormControls();
    $("#ScopeIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Scope').trigger('reset');
    $('#CreateSpartan_BR_Scope').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Scope');
...
$(document).ready(function () {
    $("form#CreateSpartan_BR_Scope").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_ScopeData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_ScopeData(function (currentId) {
                $("#ScopeIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceScopeId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Scope').find('.col-sm-8').each(function () {

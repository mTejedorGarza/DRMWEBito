...
function ClearControls() {
    ...
    $('#CreateSpartan_Report_Format')[0].reset();
    ClearFormControls();
    $("#FormatIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Report_Format').trigger('reset');
    $('#CreateSpartan_Report_Format').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Report_Format');
...
$(document).ready(function () {
    $("form#CreateSpartan_Report_Format").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Report_Format").on('click', '#Spartan_Report_FormatCancelar', function () {
        BackToGrid();
    });
	$("form#CreateSpartan_Report_Format").on('click', '#Spartan_Report_FormatGuardar', function () {
        EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Report_FormatData(function () {
                EjecutarValidacionesDespuesDeGuardar();
                BackToGrid();
            });
    });
	$("form#CreateSpartan_Report_Format").on('click', '#Spartan_Report_FormatGuardarYNuevo', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation()) {
            SendSpartan_Report_FormatData(function () {
                ClearControls();
    ...
    $("form#CreateSpartan_Report_Format").on('click', '#Spartan_Report_FormatGuardarYCopia', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Report_FormatData(function (currentId) {
                $("#FormatIdId").val("0");
...
var Spartan_Report_FormatisdisplayBusinessPropery = false;
Spartan_Report_FormatDisplayBusinessRuleButtons(Spartan_Report_FormatisdisplayBusinessPropery);
function Spartan_Report_FormatDisplayBusinessRule() {
    if (!Spartan_Report_FormatisdisplayBusinessPropery) {
        $('#CreateSpartan_Report_Format').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_Report_FormatdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Report_FormatPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Report_FormatdisplayBusinessPropery').remove();
    }
    Spartan_Report_FormatDisplayBusinessRuleButtons(!Spartan_Report_FormatisdisplayBusinessPropery);
    Spartan_Report_FormatisdisplayBusinessPropery = (Spartan_Report_FormatisdisplayBusinessPropery ? false : true);
}
function Spartan_Report_FormatDisplayBusinessRuleButtons(flag) {

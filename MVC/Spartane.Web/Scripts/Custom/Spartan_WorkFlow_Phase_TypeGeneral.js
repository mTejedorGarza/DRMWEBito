...
function ClearControls() {
    $("#ReferencePhase_TypeId").val("0");
    $('#CreateSpartan_WorkFlow_Phase_Type')[0].reset();
    ClearFormControls();
    $("#Phase_TypeIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_WorkFlow_Phase_Type').trigger('reset');
    $('#CreateSpartan_WorkFlow_Phase_Type').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_WorkFlow_Phase_Type');
...
function ResetClaveLabel() {
    $("#lblPhase_TypeId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_WorkFlow_Phase_Type").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_WorkFlow_Phase_Type").on('click', '#Spartan_WorkFlow_Phase_TypeCancelar', function () {
        Spartan_WorkFlow_Phase_TypeBackToGrid();
    });
	$("form#CreateSpartan_WorkFlow_Phase_Type").on('click', '#Spartan_WorkFlow_Phase_TypeGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_Phase_TypeData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_WorkFlow_Phase_TypeBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_WorkFlow_Phase_Type").on('click', '#Spartan_WorkFlow_Phase_TypeGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_WorkFlow_Phase_TypeData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_WorkFlow_Phase_Type").on('click', '#Spartan_WorkFlow_Phase_TypeGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_Phase_TypeData(function (currentId) {
					$("#Phase_TypeIdId").val("0");
	
					ResetClaveLabel();
					$("#ReferencePhase_TypeId").val(currentId);
...
var Spartan_WorkFlow_Phase_TypeisdisplayBusinessPropery = false;
Spartan_WorkFlow_Phase_TypeDisplayBusinessRuleButtons(Spartan_WorkFlow_Phase_TypeisdisplayBusinessPropery);
function Spartan_WorkFlow_Phase_TypeDisplayBusinessRule() {
    if (!Spartan_WorkFlow_Phase_TypeisdisplayBusinessPropery) {
        $('#CreateSpartan_WorkFlow_Phase_Type').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_WorkFlow_Phase_TypedisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_WorkFlow_Phase_TypePropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_WorkFlow_Phase_TypedisplayBusinessPropery').remove();
    }
    Spartan_WorkFlow_Phase_TypeDisplayBusinessRuleButtons(!Spartan_WorkFlow_Phase_TypeisdisplayBusinessPropery);
    Spartan_WorkFlow_Phase_TypeisdisplayBusinessPropery = (Spartan_WorkFlow_Phase_TypeisdisplayBusinessPropery ? false : true);
}
function Spartan_WorkFlow_Phase_TypeDisplayBusinessRuleButtons(flag) {

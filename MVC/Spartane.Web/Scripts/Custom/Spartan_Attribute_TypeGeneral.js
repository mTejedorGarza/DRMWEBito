...
function ClearControls() {
    $("#ReferenceAttribute_Type_Id").val("0");
    $('#CreateSpartan_Attribute_Type')[0].reset();
    ClearFormControls();
    $("#Attribute_Type_IdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Attribute_Type').trigger('reset');
    $('#CreateSpartan_Attribute_Type').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Attribute_Type');
...
function ResetClaveLabel() {
    $("#lblAttribute_Type_Id").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_Attribute_Type").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_Attribute_TypeData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_Attribute_TypeData(function (currentId) {
                $("#Attribute_Type_IdId").val("0");

                ResetClaveLabel();
                $("#ReferenceAttribute_Type_Id").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_Attribute_Type').find('.col-sm-8').each(function () {

...
function ClearControls() {
    $("#ReferenceRoles_by_StateId").val("0");
    $('#CreateSpartan_WorkFlow_Roles_by_State')[0].reset();
    ClearFormControls();
    $("#Roles_by_StateIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_WorkFlow_Roles_by_State').trigger('reset');
    $('#CreateSpartan_WorkFlow_Roles_by_State').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_WorkFlow_Roles_by_State');
...
function ResetClaveLabel() {
    $("#lblRoles_by_StateId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_WorkFlow_Roles_by_State").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_WorkFlow_Roles_by_State").on('click', '#Spartan_WorkFlow_Roles_by_StateCancelar', function () {
        Spartan_WorkFlow_Roles_by_StateBackToGrid();
    });
	$("form#CreateSpartan_WorkFlow_Roles_by_State").on('click', '#Spartan_WorkFlow_Roles_by_StateGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_Roles_by_StateData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_WorkFlow_Roles_by_StateBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_WorkFlow_Roles_by_State").on('click', '#Spartan_WorkFlow_Roles_by_StateGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_WorkFlow_Roles_by_StateData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_WorkFlow_Roles_by_State").on('click', '#Spartan_WorkFlow_Roles_by_StateGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_Roles_by_StateData(function (currentId) {
					$("#Roles_by_StateIdId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceRoles_by_StateId").val(currentId);
...
var Spartan_WorkFlow_Roles_by_StateisdisplayBusinessPropery = false;
Spartan_WorkFlow_Roles_by_StateDisplayBusinessRuleButtons(Spartan_WorkFlow_Roles_by_StateisdisplayBusinessPropery);
function Spartan_WorkFlow_Roles_by_StateDisplayBusinessRule() {
    if (!Spartan_WorkFlow_Roles_by_StateisdisplayBusinessPropery) {
        $('#CreateSpartan_WorkFlow_Roles_by_State').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_WorkFlow_Roles_by_StatedisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_WorkFlow_Roles_by_StatePropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_WorkFlow_Roles_by_StatedisplayBusinessPropery').remove();
    }
    Spartan_WorkFlow_Roles_by_StateDisplayBusinessRuleButtons(!Spartan_WorkFlow_Roles_by_StateisdisplayBusinessPropery);
    Spartan_WorkFlow_Roles_by_StateisdisplayBusinessPropery = (Spartan_WorkFlow_Roles_by_StateisdisplayBusinessPropery ? false : true);
}
function Spartan_WorkFlow_Roles_by_StateDisplayBusinessRuleButtons(flag) {

...
function ClearControls() {
    $("#ReferenceKey_Type_History").val("0");
    $('#CreateSpartan_BR_Type_History')[0].reset();
    ClearFormControls();
    $("#Key_Type_HistoryId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Type_History').trigger('reset');
    $('#CreateSpartan_BR_Type_History').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Type_History');
...
function ResetClaveLabel() {
    $("#lblKey_Type_History").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Type_History").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_BR_Type_History").on('click', '#Spartan_BR_Type_HistoryCancelar', function () {
        Spartan_BR_Type_HistoryBackToGrid();
    });
	$("form#CreateSpartan_BR_Type_History").on('click', '#Spartan_BR_Type_HistoryGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_BR_Type_HistoryData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_BR_Type_HistoryBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_BR_Type_History").on('click', '#Spartan_BR_Type_HistoryGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_BR_Type_HistoryData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_BR_Type_History").on('click', '#Spartan_BR_Type_HistoryGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_BR_Type_HistoryData(function (currentId) {
					$("#Key_Type_HistoryId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceKey_Type_History").val(currentId);
...
var Spartan_BR_Type_HistoryisdisplayBusinessPropery = false;
Spartan_BR_Type_HistoryDisplayBusinessRuleButtons(Spartan_BR_Type_HistoryisdisplayBusinessPropery);
function Spartan_BR_Type_HistoryDisplayBusinessRule() {
    if (!Spartan_BR_Type_HistoryisdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Type_History').find('.col-sm-8').each(function () {
			var div =$(this); 
            ...
            mainElementAttributes += '<div class="Spartan_BR_Type_HistorydisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_BR_Type_HistoryPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_BR_Type_HistorydisplayBusinessPropery').remove();
    }
    Spartan_BR_Type_HistoryDisplayBusinessRuleButtons(!Spartan_BR_Type_HistoryisdisplayBusinessPropery);
    Spartan_BR_Type_HistoryisdisplayBusinessPropery = (Spartan_BR_Type_HistoryisdisplayBusinessPropery ? false : true);
}
function Spartan_BR_Type_HistoryDisplayBusinessRuleButtons(flag) {

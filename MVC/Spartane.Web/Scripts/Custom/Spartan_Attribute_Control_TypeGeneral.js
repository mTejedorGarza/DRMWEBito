...
function ClearControls() {
    $("#ReferenceControlTypeId").val("0");
    $('#CreateSpartan_Attribute_Control_Type')[0].reset();
    ClearFormControls();
    $("#ControlTypeIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Attribute_Control_Type').trigger('reset');
    $('#CreateSpartan_Attribute_Control_Type').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Attribute_Control_Type');
...
function ResetClaveLabel() {
    $("#lblControlTypeId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_Attribute_Control_Type").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_Attribute_Control_TypeData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_Attribute_Control_TypeData(function (currentId) {
                $("#ControlTypeIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceControlTypeId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_Attribute_Control_Type').find('.col-sm-8').each(function () {

...
function ClearControls() {
    $("#ReferenceProcessEventId").val("0");
    $('#CreateSpartan_BR_Process_Event')[0].reset();
    ClearFormControls();
    $("#ProcessEventIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Process_Event').trigger('reset');
    $('#CreateSpartan_BR_Process_Event').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Process_Event');
...
function ResetClaveLabel() {
    $("#lblProcessEventId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Process_Event").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Process_EventData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Process_EventData(function (currentId) {
                $("#ProcessEventIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceProcessEventId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Process_Event').find('.col-sm-8').each(function () {

...
function ClearControls() {
    $("#ReferenceIdTraductionDetail").val("0");
    $('#CreateSpartan_Traduction_Detail')[0].reset();
    ClearFormControls();
    $("#IdTraductionDetailId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Traduction_Detail').trigger('reset');
    $('#CreateSpartan_Traduction_Detail').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Traduction_Detail');
...
function ResetClaveLabel() {
    $("#lblIdTraductionDetail").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_Traduction_Detail").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Traduction_Detail").on('click', '#Spartan_Traduction_DetailCancelar', function () {
        Spartan_Traduction_DetailBackToGrid();
    });
	$("form#CreateSpartan_Traduction_Detail").on('click', '#Spartan_Traduction_DetailGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Traduction_DetailData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_Traduction_DetailBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_Traduction_Detail").on('click', '#Spartan_Traduction_DetailGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_Traduction_DetailData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_Traduction_Detail").on('click', '#Spartan_Traduction_DetailGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Traduction_DetailData(function (currentId) {
					$("#IdTraductionDetailId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceIdTraductionDetail").val(currentId);
...
var Spartan_Traduction_DetailisdisplayBusinessPropery = false;
Spartan_Traduction_DetailDisplayBusinessRuleButtons(Spartan_Traduction_DetailisdisplayBusinessPropery);
function Spartan_Traduction_DetailDisplayBusinessRule() {
    if (!Spartan_Traduction_DetailisdisplayBusinessPropery) {
        $('#CreateSpartan_Traduction_Detail').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_Traduction_DetaildisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Traduction_DetailPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Traduction_DetaildisplayBusinessPropery').remove();
    }
    Spartan_Traduction_DetailDisplayBusinessRuleButtons(!Spartan_Traduction_DetailisdisplayBusinessPropery);
    Spartan_Traduction_DetailisdisplayBusinessPropery = (Spartan_Traduction_DetailisdisplayBusinessPropery ? false : true);
}
function Spartan_Traduction_DetailDisplayBusinessRuleButtons(flag) {

...
function ClearControls() {
    $("#ReferenceOperatorId").val("0");
    $('#CreateSpartan_WorkFlow_Operator')[0].reset();
    ClearFormControls();
    $("#OperatorIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_WorkFlow_Operator').trigger('reset');
    $('#CreateSpartan_WorkFlow_Operator').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_WorkFlow_Operator');
...
function ResetClaveLabel() {
    $("#lblOperatorId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_WorkFlow_Operator").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_WorkFlow_Operator").on('click', '#Spartan_WorkFlow_OperatorCancelar', function () {
        Spartan_WorkFlow_OperatorBackToGrid();
    });
	$("form#CreateSpartan_WorkFlow_Operator").on('click', '#Spartan_WorkFlow_OperatorGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_OperatorData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_WorkFlow_OperatorBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_WorkFlow_Operator").on('click', '#Spartan_WorkFlow_OperatorGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_WorkFlow_OperatorData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_WorkFlow_Operator").on('click', '#Spartan_WorkFlow_OperatorGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_OperatorData(function (currentId) {
					$("#OperatorIdId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceOperatorId").val(currentId);
...
var Spartan_WorkFlow_OperatorisdisplayBusinessPropery = false;
Spartan_WorkFlow_OperatorDisplayBusinessRuleButtons(Spartan_WorkFlow_OperatorisdisplayBusinessPropery);
function Spartan_WorkFlow_OperatorDisplayBusinessRule() {
    if (!Spartan_WorkFlow_OperatorisdisplayBusinessPropery) {
        $('#CreateSpartan_WorkFlow_Operator').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_WorkFlow_OperatordisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_WorkFlow_OperatorPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_WorkFlow_OperatordisplayBusinessPropery').remove();
    }
    Spartan_WorkFlow_OperatorDisplayBusinessRuleButtons(!Spartan_WorkFlow_OperatorisdisplayBusinessPropery);
    Spartan_WorkFlow_OperatorisdisplayBusinessPropery = (Spartan_WorkFlow_OperatorisdisplayBusinessPropery ? false : true);
}
function Spartan_WorkFlow_OperatorDisplayBusinessRuleButtons(flag) {

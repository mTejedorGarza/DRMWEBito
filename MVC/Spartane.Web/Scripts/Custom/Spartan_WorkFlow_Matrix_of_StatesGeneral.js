...
function ClearControls() {
    $("#ReferenceMatrix_of_StatesId").val("0");
    $('#CreateSpartan_WorkFlow_Matrix_of_States')[0].reset();
    ClearFormControls();
    $("#Matrix_of_StatesIdId").val("0");
    $('#Attribute').empty();
    $("#Attribute").append('<option value=""></option>');
...
function ClearFormControls() {
    $('#CreateSpartan_WorkFlow_Matrix_of_States').trigger('reset');
    $('#CreateSpartan_WorkFlow_Matrix_of_States').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_WorkFlow_Matrix_of_States');
...
function ResetClaveLabel() {
    $("#lblMatrix_of_StatesId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_WorkFlow_Matrix_of_States").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_WorkFlow_Matrix_of_States").on('click', '#Spartan_WorkFlow_Matrix_of_StatesCancelar', function () {
        Spartan_WorkFlow_Matrix_of_StatesBackToGrid();
    });
	$("form#CreateSpartan_WorkFlow_Matrix_of_States").on('click', '#Spartan_WorkFlow_Matrix_of_StatesGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_Matrix_of_StatesData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_WorkFlow_Matrix_of_StatesBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_WorkFlow_Matrix_of_States").on('click', '#Spartan_WorkFlow_Matrix_of_StatesGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_WorkFlow_Matrix_of_StatesData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_WorkFlow_Matrix_of_States").on('click', '#Spartan_WorkFlow_Matrix_of_StatesGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_Matrix_of_StatesData(function (currentId) {
					$("#Matrix_of_StatesIdId").val("0");
	    $('#Attribute').empty();
    $("#Attribute").append('<option value=""></option>');
    $('#Attribute').val('0').trigger('change');
					...
					ResetClaveLabel();
					$("#ReferenceMatrix_of_StatesId").val(currentId);
...
var Spartan_WorkFlow_Matrix_of_StatesisdisplayBusinessPropery = false;
Spartan_WorkFlow_Matrix_of_StatesDisplayBusinessRuleButtons(Spartan_WorkFlow_Matrix_of_StatesisdisplayBusinessPropery);
function Spartan_WorkFlow_Matrix_of_StatesDisplayBusinessRule() {
    if (!Spartan_WorkFlow_Matrix_of_StatesisdisplayBusinessPropery) {
        $('#CreateSpartan_WorkFlow_Matrix_of_States').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_WorkFlow_Matrix_of_StatesdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_WorkFlow_Matrix_of_StatesPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_WorkFlow_Matrix_of_StatesdisplayBusinessPropery').remove();
    }
    Spartan_WorkFlow_Matrix_of_StatesDisplayBusinessRuleButtons(!Spartan_WorkFlow_Matrix_of_StatesisdisplayBusinessPropery);
    Spartan_WorkFlow_Matrix_of_StatesisdisplayBusinessPropery = (Spartan_WorkFlow_Matrix_of_StatesisdisplayBusinessPropery ? false : true);
}
function Spartan_WorkFlow_Matrix_of_StatesDisplayBusinessRuleButtons(flag) {

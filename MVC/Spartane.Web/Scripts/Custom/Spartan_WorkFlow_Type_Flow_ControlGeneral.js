...
function ClearControls() {
    $("#ReferenceType_Flow_ControlId").val("0");
    $('#CreateSpartan_WorkFlow_Type_Flow_Control')[0].reset();
    ClearFormControls();
    $("#Type_Flow_ControlIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_WorkFlow_Type_Flow_Control').trigger('reset');
    $('#CreateSpartan_WorkFlow_Type_Flow_Control').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_WorkFlow_Type_Flow_Control');
...
function ResetClaveLabel() {
    $("#lblType_Flow_ControlId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_WorkFlow_Type_Flow_Control").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_WorkFlow_Type_Flow_Control").on('click', '#Spartan_WorkFlow_Type_Flow_ControlCancelar', function () {
        Spartan_WorkFlow_Type_Flow_ControlBackToGrid();
    });
	$("form#CreateSpartan_WorkFlow_Type_Flow_Control").on('click', '#Spartan_WorkFlow_Type_Flow_ControlGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_Type_Flow_ControlData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_WorkFlow_Type_Flow_ControlBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_WorkFlow_Type_Flow_Control").on('click', '#Spartan_WorkFlow_Type_Flow_ControlGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_WorkFlow_Type_Flow_ControlData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_WorkFlow_Type_Flow_Control").on('click', '#Spartan_WorkFlow_Type_Flow_ControlGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_Type_Flow_ControlData(function (currentId) {
					$("#Type_Flow_ControlIdId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceType_Flow_ControlId").val(currentId);
...
var Spartan_WorkFlow_Type_Flow_ControlisdisplayBusinessPropery = false;
Spartan_WorkFlow_Type_Flow_ControlDisplayBusinessRuleButtons(Spartan_WorkFlow_Type_Flow_ControlisdisplayBusinessPropery);
function Spartan_WorkFlow_Type_Flow_ControlDisplayBusinessRule() {
    if (!Spartan_WorkFlow_Type_Flow_ControlisdisplayBusinessPropery) {
        $('#CreateSpartan_WorkFlow_Type_Flow_Control').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_WorkFlow_Type_Flow_ControldisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_WorkFlow_Type_Flow_ControlPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_WorkFlow_Type_Flow_ControldisplayBusinessPropery').remove();
    }
    Spartan_WorkFlow_Type_Flow_ControlDisplayBusinessRuleButtons(!Spartan_WorkFlow_Type_Flow_ControlisdisplayBusinessPropery);
    Spartan_WorkFlow_Type_Flow_ControlisdisplayBusinessPropery = (Spartan_WorkFlow_Type_Flow_ControlisdisplayBusinessPropery ? false : true);
}
function Spartan_WorkFlow_Type_Flow_ControlDisplayBusinessRuleButtons(flag) {

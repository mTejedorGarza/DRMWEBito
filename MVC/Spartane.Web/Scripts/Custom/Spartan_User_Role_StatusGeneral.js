...
function ClearControls() {
    $("#ReferenceUser_Role_Status_Id").val("0");
    $('#CreateSpartan_User_Role_Status')[0].reset();
    ClearFormControls();
    $("#User_Role_Status_IdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_User_Role_Status').trigger('reset');
    $('#CreateSpartan_User_Role_Status').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_User_Role_Status');
...
function ResetClaveLabel() {
    $("#lblUser_Role_Status_Id").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_User_Role_Status").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_User_Role_Status").on('click', '#Spartan_User_Role_StatusCancelar', function () {
        Spartan_User_Role_StatusBackToGrid();
    });
	$("form#CreateSpartan_User_Role_Status").on('click', '#Spartan_User_Role_StatusGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_User_Role_StatusData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_User_Role_StatusBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_User_Role_Status").on('click', '#Spartan_User_Role_StatusGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_User_Role_StatusData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_User_Role_Status").on('click', '#Spartan_User_Role_StatusGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_User_Role_StatusData(function (currentId) {
					$("#User_Role_Status_IdId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceUser_Role_Status_Id").val(currentId);
...
var Spartan_User_Role_StatusisdisplayBusinessPropery = false;
Spartan_User_Role_StatusDisplayBusinessRuleButtons(Spartan_User_Role_StatusisdisplayBusinessPropery);
function Spartan_User_Role_StatusDisplayBusinessRule() {
    if (!Spartan_User_Role_StatusisdisplayBusinessPropery) {
        $('#CreateSpartan_User_Role_Status').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_User_Role_StatusdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_User_Role_StatusPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_User_Role_StatusdisplayBusinessPropery').remove();
    }
    Spartan_User_Role_StatusDisplayBusinessRuleButtons(!Spartan_User_Role_StatusisdisplayBusinessPropery);
    Spartan_User_Role_StatusisdisplayBusinessPropery = (Spartan_User_Role_StatusisdisplayBusinessPropery ? false : true);
}
function Spartan_User_Role_StatusDisplayBusinessRuleButtons(flag) {

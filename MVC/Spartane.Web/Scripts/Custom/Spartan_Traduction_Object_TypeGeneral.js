...
function ClearControls() {
    $("#ReferenceIdType").val("0");
    $('#CreateSpartan_Traduction_Object_Type')[0].reset();
    ClearFormControls();
    $("#IdTypeId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Traduction_Object_Type').trigger('reset');
    $('#CreateSpartan_Traduction_Object_Type').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Traduction_Object_Type');
...
$(document).ready(function () {
    $("form#CreateSpartan_Traduction_Object_Type").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Traduction_Object_Type").on('click', '#Spartan_Traduction_Object_TypeCancelar', function () {
        Spartan_Traduction_Object_TypeBackToGrid();
    });
	$("form#CreateSpartan_Traduction_Object_Type").on('click', '#Spartan_Traduction_Object_TypeGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Traduction_Object_TypeData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_Traduction_Object_TypeBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_Traduction_Object_Type").on('click', '#Spartan_Traduction_Object_TypeGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_Traduction_Object_TypeData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_Traduction_Object_Type").on('click', '#Spartan_Traduction_Object_TypeGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Traduction_Object_TypeData(function (currentId) {
					$("#IdTypeId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceIdType").val(currentId);
...
var Spartan_Traduction_Object_TypeisdisplayBusinessPropery = false;
Spartan_Traduction_Object_TypeDisplayBusinessRuleButtons(Spartan_Traduction_Object_TypeisdisplayBusinessPropery);
function Spartan_Traduction_Object_TypeDisplayBusinessRule() {
    if (!Spartan_Traduction_Object_TypeisdisplayBusinessPropery) {
        $('#CreateSpartan_Traduction_Object_Type').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_Traduction_Object_TypedisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Traduction_Object_TypePropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Traduction_Object_TypedisplayBusinessPropery').remove();
    }
    Spartan_Traduction_Object_TypeDisplayBusinessRuleButtons(!Spartan_Traduction_Object_TypeisdisplayBusinessPropery);
    Spartan_Traduction_Object_TypeisdisplayBusinessPropery = (Spartan_Traduction_Object_TypeisdisplayBusinessPropery ? false : true);
}
function Spartan_Traduction_Object_TypeDisplayBusinessRuleButtons(flag) {

...
function ClearControls() {
    $("#ReferenceFormat").val("0");
    $('#CreateSpartan_Format_Configuration')[0].reset();
    ClearFormControls();
    $("#FormatId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Format_Configuration').trigger('reset');
    $('#CreateSpartan_Format_Configuration').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Format_Configuration');
...
$(document).ready(function () {
    $("form#CreateSpartan_Format_Configuration").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Format_Configuration").on('click', '#Spartan_Format_ConfigurationCancelar', function () {
        BackToGrid();
    });
	$("form#CreateSpartan_Format_Configuration").on('click', '#Spartan_Format_ConfigurationGuardar', function () {
        EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Format_ConfigurationData(function () {
                EjecutarValidacionesDespuesDeGuardar();
                BackToGrid();
            });
    });
	$("form#CreateSpartan_Format_Configuration").on('click', '#Spartan_Format_ConfigurationGuardarYNuevo', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation()) {
            SendSpartan_Format_ConfigurationData(function () {
                ClearControls();
    ...
    $("form#CreateSpartan_Format_Configuration").on('click', '#Spartan_Format_ConfigurationGuardarYCopia', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Format_ConfigurationData(function (currentId) {
                $("#FormatId").val("0");

                ResetClaveLabel();
                $("#ReferenceFormat").val(currentId);
...
var Spartan_Format_ConfigurationisdisplayBusinessPropery = false;
Spartan_Format_ConfigurationDisplayBusinessRuleButtons(Spartan_Format_ConfigurationisdisplayBusinessPropery);
function Spartan_Format_ConfigurationDisplayBusinessRule() {
    if (!Spartan_Format_ConfigurationisdisplayBusinessPropery) {
        $('#CreateSpartan_Format_Configuration').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_Format_ConfigurationdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Format_ConfigurationPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Format_ConfigurationdisplayBusinessPropery').remove();
    }
    Spartan_Format_ConfigurationDisplayBusinessRuleButtons(!Spartan_Format_ConfigurationisdisplayBusinessPropery);
    Spartan_Format_ConfigurationisdisplayBusinessPropery = (Spartan_Format_ConfigurationisdisplayBusinessPropery ? false : true);
}
function Spartan_Format_ConfigurationDisplayBusinessRuleButtons(flag) {

...
function ClearControls() {
    ...
    $('#CreateSpartan_BR_Condition')[0].reset();
    ClearFormControls();
    $("#ConditionIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Condition').trigger('reset');
    $('#CreateSpartan_BR_Condition').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Condition');
...
$(document).ready(function () {
    $("form#CreateSpartan_BR_Condition").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_ConditionData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_ConditionData(function (currentId) {
                $("#ConditionIdId").val("0");
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Condition').find('.col-sm-8').each(function () {

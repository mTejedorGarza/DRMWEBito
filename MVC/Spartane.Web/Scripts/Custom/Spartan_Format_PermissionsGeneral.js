...
function ClearControls() {
    $("#ReferencePermissionId").val("0");
    $('#CreateSpartan_Format_Permissions')[0].reset();
    ClearFormControls();
    $("#PermissionIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Format_Permissions').trigger('reset');
    $('#CreateSpartan_Format_Permissions').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Format_Permissions');
...
function ResetClaveLabel() {
    $("#lblPermissionId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_Format_Permissions").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Format_Permissions").on('click', '#Spartan_Format_PermissionsCancelar', function () {
        BackToGrid();
    });
	$("form#CreateSpartan_Format_Permissions").on('click', '#Spartan_Format_PermissionsGuardar', function () {
        EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Format_PermissionsData(function () {
                EjecutarValidacionesDespuesDeGuardar();
                BackToGrid();
            });
    });
	$("form#CreateSpartan_Format_Permissions").on('click', '#Spartan_Format_PermissionsGuardarYNuevo', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation()) {
            SendSpartan_Format_PermissionsData(function () {
                ClearControls();
    ...
    $("form#CreateSpartan_Format_Permissions").on('click', '#Spartan_Format_PermissionsGuardarYCopia', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Format_PermissionsData(function (currentId) {
                $("#PermissionIdId").val("0");

                ResetClaveLabel();
                $("#ReferencePermissionId").val(currentId);
...
var Spartan_Format_PermissionsisdisplayBusinessPropery = false;
Spartan_Format_PermissionsDisplayBusinessRuleButtons(Spartan_Format_PermissionsisdisplayBusinessPropery);
function Spartan_Format_PermissionsDisplayBusinessRule() {
    if (!Spartan_Format_PermissionsisdisplayBusinessPropery) {
        $('#CreateSpartan_Format_Permissions').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_Format_PermissionsdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Format_PermissionsPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Format_PermissionsdisplayBusinessPropery').remove();
    }
    Spartan_Format_PermissionsDisplayBusinessRuleButtons(!Spartan_Format_PermissionsisdisplayBusinessPropery);
    Spartan_Format_PermissionsisdisplayBusinessPropery = (Spartan_Format_PermissionsisdisplayBusinessPropery ? false : true);
}
function Spartan_Format_PermissionsDisplayBusinessRuleButtons(flag) {

...
function ClearControls() {
    $("#ReferenceProcessEventDetailId").val("0");
    $('#CreateSpartan_BR_Process_Event_Detail')[0].reset();
    ClearFormControls();
    $("#ProcessEventDetailIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Process_Event_Detail').trigger('reset');
    $('#CreateSpartan_BR_Process_Event_Detail').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Process_Event_Detail');
...
function ResetClaveLabel() {
    $("#lblProcessEventDetailId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Process_Event_Detail").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Process_Event_DetailData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Process_Event_DetailData(function (currentId) {
                $("#ProcessEventDetailIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceProcessEventDetailId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Process_Event_Detail').find('.col-sm-8').each(function () {

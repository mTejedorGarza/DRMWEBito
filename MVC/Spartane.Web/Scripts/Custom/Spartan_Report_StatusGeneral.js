...
function ClearControls() {
    ...
    $('#CreateSpartan_Report_Status')[0].reset();
    ClearFormControls();
    $("#StatusIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Report_Status').trigger('reset');
    $('#CreateSpartan_Report_Status').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Report_Status');
...
$(document).ready(function () {
    $("form#CreateSpartan_Report_Status").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Report_Status").on('click', '#Spartan_Report_StatusCancelar', function () {
        BackToGrid();
    });
	$("form#CreateSpartan_Report_Status").on('click', '#Spartan_Report_StatusGuardar', function () {
        EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Report_StatusData(function () {
                EjecutarValidacionesDespuesDeGuardar();
                BackToGrid();
            });
    });
	$("form#CreateSpartan_Report_Status").on('click', '#Spartan_Report_StatusGuardarYNuevo', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation()) {
            SendSpartan_Report_StatusData(function () {
                ClearControls();
    ...
    $("form#CreateSpartan_Report_Status").on('click', '#Spartan_Report_StatusGuardarYCopia', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Report_StatusData(function (currentId) {
                $("#StatusIdId").val("0");
...
var Spartan_Report_StatusisdisplayBusinessPropery = false;
Spartan_Report_StatusDisplayBusinessRuleButtons(Spartan_Report_StatusisdisplayBusinessPropery);
function Spartan_Report_StatusDisplayBusinessRule() {
    if (!Spartan_Report_StatusisdisplayBusinessPropery) {
        $('#CreateSpartan_Report_Status').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_Report_StatusdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Report_StatusPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Report_StatusdisplayBusinessPropery').remove();
    }
    Spartan_Report_StatusDisplayBusinessRuleButtons(!Spartan_Report_StatusisdisplayBusinessPropery);
    Spartan_Report_StatusisdisplayBusinessPropery = (Spartan_Report_StatusisdisplayBusinessPropery ? false : true);
}
function Spartan_Report_StatusDisplayBusinessRuleButtons(flag) {

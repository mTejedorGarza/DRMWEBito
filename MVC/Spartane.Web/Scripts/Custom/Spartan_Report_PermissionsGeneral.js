...
function ClearControls() {
    $("#ReferenceReportPermissionId").val("0");
    $('#CreateSpartan_Report_Permissions')[0].reset();
    ClearFormControls();
    $("#ReportPermissionIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Report_Permissions').trigger('reset');
    $('#CreateSpartan_Report_Permissions').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Report_Permissions');
...
function ResetClaveLabel() {
    $("#lblReportPermissionId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_Report_Permissions").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Report_Permissions").on('click', '#Spartan_Report_PermissionsCancelar', function () {
        BackToGrid();
    });
	$("form#CreateSpartan_Report_Permissions").on('click', '#Spartan_Report_PermissionsGuardar', function () {
        EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Report_PermissionsData(function () {
                EjecutarValidacionesDespuesDeGuardar();
                BackToGrid();
            });
    });
	$("form#CreateSpartan_Report_Permissions").on('click', '#Spartan_Report_PermissionsGuardarYNuevo', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation()) {
            SendSpartan_Report_PermissionsData(function () {
                ClearControls();
    ...
    $("form#CreateSpartan_Report_Permissions").on('click', '#Spartan_Report_PermissionsGuardarYCopia', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_Report_PermissionsData(function (currentId) {
                $("#ReportPermissionIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceReportPermissionId").val(currentId);
...
var Spartan_Report_PermissionsisdisplayBusinessPropery = false;
Spartan_Report_PermissionsDisplayBusinessRuleButtons(Spartan_Report_PermissionsisdisplayBusinessPropery);
function Spartan_Report_PermissionsDisplayBusinessRule() {
    if (!Spartan_Report_PermissionsisdisplayBusinessPropery) {
        $('#CreateSpartan_Report_Permissions').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_Report_PermissionsdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Report_PermissionsPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Report_PermissionsdisplayBusinessPropery').remove();
    }
    Spartan_Report_PermissionsDisplayBusinessRuleButtons(!Spartan_Report_PermissionsisdisplayBusinessPropery);
    Spartan_Report_PermissionsisdisplayBusinessPropery = (Spartan_Report_PermissionsisdisplayBusinessPropery ? false : true);
}
function Spartan_Report_PermissionsDisplayBusinessRuleButtons(flag) {

...
function ClearControls() {
    ...
    $('#CreateSpartan_User_Historical_Password')[0].reset();
    ClearFormControls();
    $("#ClaveId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_User_Historical_Password').trigger('reset');
    $('#CreateSpartan_User_Historical_Password').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_User_Historical_Password');
...
$(document).ready(function () {
    $("form#CreateSpartan_User_Historical_Password").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_User_Historical_Password").on('click', '#Spartan_User_Historical_PasswordCancelar', function () {
        Spartan_User_Historical_PasswordBackToGrid();
    });
	$("form#CreateSpartan_User_Historical_Password").on('click', '#Spartan_User_Historical_PasswordGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_User_Historical_PasswordData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_User_Historical_PasswordBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_User_Historical_Password").on('click', '#Spartan_User_Historical_PasswordGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_User_Historical_PasswordData(function () {
				EjecutarValidacionesDespuesDeGuardar();		
    ...
    $("form#CreateSpartan_User_Historical_Password").on('click', '#Spartan_User_Historical_PasswordGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_User_Historical_PasswordData(function (currentId) {
					$("#ClaveId").val("0");
...
var Spartan_User_Historical_PasswordisdisplayBusinessPropery = false;
Spartan_User_Historical_PasswordDisplayBusinessRuleButtons(Spartan_User_Historical_PasswordisdisplayBusinessPropery);
function Spartan_User_Historical_PasswordDisplayBusinessRule() {
    if (!Spartan_User_Historical_PasswordisdisplayBusinessPropery) {
        $('#CreateSpartan_User_Historical_Password').find('.col-sm-8').each(function () {
			var div =$(this); 
            ...
            mainElementAttributes += '<div class="Spartan_User_Historical_PassworddisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_User_Historical_PasswordPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_User_Historical_PassworddisplayBusinessPropery').remove();
    }
    Spartan_User_Historical_PasswordDisplayBusinessRuleButtons(!Spartan_User_Historical_PasswordisdisplayBusinessPropery);
    Spartan_User_Historical_PasswordisdisplayBusinessPropery = (Spartan_User_Historical_PasswordisdisplayBusinessPropery ? false : true);
}
function Spartan_User_Historical_PasswordDisplayBusinessRuleButtons(flag) {

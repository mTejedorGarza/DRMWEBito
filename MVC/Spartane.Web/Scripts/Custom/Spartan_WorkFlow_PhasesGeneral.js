...
function ClearControls() {
    $("#ReferencePhasesId").val("0");
    $('#CreateSpartan_WorkFlow_Phases')[0].reset();
    ClearFormControls();
    $("#PhasesIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_WorkFlow_Phases').trigger('reset');
    $('#CreateSpartan_WorkFlow_Phases').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_WorkFlow_Phases');
...
$(document).ready(function () {
    $("form#CreateSpartan_WorkFlow_Phases").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_WorkFlow_Phases").on('click', '#Spartan_WorkFlow_PhasesCancelar', function () {
        Spartan_WorkFlow_PhasesBackToGrid();
    });
	$("form#CreateSpartan_WorkFlow_Phases").on('click', '#Spartan_WorkFlow_PhasesGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_PhasesData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_WorkFlow_PhasesBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_WorkFlow_Phases").on('click', '#Spartan_WorkFlow_PhasesGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_WorkFlow_PhasesData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_WorkFlow_Phases").on('click', '#Spartan_WorkFlow_PhasesGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_PhasesData(function (currentId) {
					$("#PhasesIdId").val("0");
	
					ResetClaveLabel();
					$("#ReferencePhasesId").val(currentId);
...
var Spartan_WorkFlow_PhasesisdisplayBusinessPropery = false;
Spartan_WorkFlow_PhasesDisplayBusinessRuleButtons(Spartan_WorkFlow_PhasesisdisplayBusinessPropery);
function Spartan_WorkFlow_PhasesDisplayBusinessRule() {
    if (!Spartan_WorkFlow_PhasesisdisplayBusinessPropery) {
        $('#CreateSpartan_WorkFlow_Phases').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_WorkFlow_PhasesdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_WorkFlow_PhasesPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_WorkFlow_PhasesdisplayBusinessPropery').remove();
    }
    Spartan_WorkFlow_PhasesDisplayBusinessRuleButtons(!Spartan_WorkFlow_PhasesisdisplayBusinessPropery);
    Spartan_WorkFlow_PhasesisdisplayBusinessPropery = (Spartan_WorkFlow_PhasesisdisplayBusinessPropery ? false : true);
}
function Spartan_WorkFlow_PhasesDisplayBusinessRuleButtons(flag) {

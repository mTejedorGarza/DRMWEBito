...
function ClearControls() {
    $("#ReferenceConditionOperatorId").val("0");
    $('#CreateSpartan_BR_Condition_Operator')[0].reset();
    ClearFormControls();
    $("#ConditionOperatorIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Condition_Operator').trigger('reset');
    $('#CreateSpartan_BR_Condition_Operator').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Condition_Operator');
...
function ResetClaveLabel() {
    $("#lblConditionOperatorId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Condition_Operator").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Condition_OperatorData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Condition_OperatorData(function (currentId) {
                $("#ConditionOperatorIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceConditionOperatorId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Condition_Operator').find('.col-sm-8').each(function () {

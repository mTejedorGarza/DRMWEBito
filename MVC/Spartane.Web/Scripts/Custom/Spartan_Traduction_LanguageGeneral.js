...
function ClearControls() {
    $("#ReferenceIdLanguage").val("0");
    $('#CreateSpartan_Traduction_Language')[0].reset();
    ClearFormControls();
    $("#IdLanguageId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Traduction_Language').trigger('reset');
    $('#CreateSpartan_Traduction_Language').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Traduction_Language');
...
function ResetClaveLabel() {
    $("#lblIdLanguage").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_Traduction_Language").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Traduction_Language").on('click', '#Spartan_Traduction_LanguageCancelar', function () {
        Spartan_Traduction_LanguageBackToGrid();
    });
	$("form#CreateSpartan_Traduction_Language").on('click', '#Spartan_Traduction_LanguageGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Traduction_LanguageData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_Traduction_LanguageBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_Traduction_Language").on('click', '#Spartan_Traduction_LanguageGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_Traduction_LanguageData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_Traduction_Language").on('click', '#Spartan_Traduction_LanguageGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Traduction_LanguageData(function (currentId) {
					$("#IdLanguageId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceIdLanguage").val(currentId);
...
var Spartan_Traduction_LanguageisdisplayBusinessPropery = false;
Spartan_Traduction_LanguageDisplayBusinessRuleButtons(Spartan_Traduction_LanguageisdisplayBusinessPropery);
function Spartan_Traduction_LanguageDisplayBusinessRule() {
    if (!Spartan_Traduction_LanguageisdisplayBusinessPropery) {
        $('#CreateSpartan_Traduction_Language').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_Traduction_LanguagedisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Traduction_LanguagePropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Traduction_LanguagedisplayBusinessPropery').remove();
    }
    Spartan_Traduction_LanguageDisplayBusinessRuleButtons(!Spartan_Traduction_LanguageisdisplayBusinessPropery);
    Spartan_Traduction_LanguageisdisplayBusinessPropery = (Spartan_Traduction_LanguageisdisplayBusinessPropery ? false : true);
}
function Spartan_Traduction_LanguageDisplayBusinessRuleButtons(flag) {

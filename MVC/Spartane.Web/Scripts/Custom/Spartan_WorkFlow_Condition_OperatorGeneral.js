...
function ClearControls() {
    $("#ReferenceCondition_OperatorId").val("0");
    $('#CreateSpartan_WorkFlow_Condition_Operator')[0].reset();
    ClearFormControls();
    $("#Condition_OperatorIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_WorkFlow_Condition_Operator').trigger('reset');
    $('#CreateSpartan_WorkFlow_Condition_Operator').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_WorkFlow_Condition_Operator');
...
function ResetClaveLabel() {
    $("#lblCondition_OperatorId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_WorkFlow_Condition_Operator").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_WorkFlow_Condition_Operator").on('click', '#Spartan_WorkFlow_Condition_OperatorCancelar', function () {
        Spartan_WorkFlow_Condition_OperatorBackToGrid();
    });
	$("form#CreateSpartan_WorkFlow_Condition_Operator").on('click', '#Spartan_WorkFlow_Condition_OperatorGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_Condition_OperatorData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_WorkFlow_Condition_OperatorBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_WorkFlow_Condition_Operator").on('click', '#Spartan_WorkFlow_Condition_OperatorGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_WorkFlow_Condition_OperatorData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_WorkFlow_Condition_Operator").on('click', '#Spartan_WorkFlow_Condition_OperatorGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_Condition_OperatorData(function (currentId) {
					$("#Condition_OperatorIdId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceCondition_OperatorId").val(currentId);
...
var Spartan_WorkFlow_Condition_OperatorisdisplayBusinessPropery = false;
Spartan_WorkFlow_Condition_OperatorDisplayBusinessRuleButtons(Spartan_WorkFlow_Condition_OperatorisdisplayBusinessPropery);
function Spartan_WorkFlow_Condition_OperatorDisplayBusinessRule() {
    if (!Spartan_WorkFlow_Condition_OperatorisdisplayBusinessPropery) {
        $('#CreateSpartan_WorkFlow_Condition_Operator').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_WorkFlow_Condition_OperatordisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_WorkFlow_Condition_OperatorPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_WorkFlow_Condition_OperatordisplayBusinessPropery').remove();
    }
    Spartan_WorkFlow_Condition_OperatorDisplayBusinessRuleButtons(!Spartan_WorkFlow_Condition_OperatorisdisplayBusinessPropery);
    Spartan_WorkFlow_Condition_OperatorisdisplayBusinessPropery = (Spartan_WorkFlow_Condition_OperatorisdisplayBusinessPropery ? false : true);
}
function Spartan_WorkFlow_Condition_OperatorDisplayBusinessRuleButtons(flag) {

...
function ClearControls() {
    $("#ReferenceId_User").val("0");
    $('#CreateSpartan_User')[0].reset();
    ClearFormControls();
    $("#Id_UserId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_User').trigger('reset');
    $('#CreateSpartan_User').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
$(document).ready(function () {
    $("form#CreateSpartan_User").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_User").on('click', '#Spartan_UserCancelar', function () {
        Spartan_UserBackToGrid();
    });
	$("form#CreateSpartan_User").on('click', '#Spartan_UserGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_UserData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_UserBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_User").on('click', '#Spartan_UserGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_UserData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_User").on('click', '#Spartan_UserGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_UserData(function (currentId) {
					$("#Id_UserId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceId_User").val(currentId);
...
var Spartan_UserisdisplayBusinessPropery = false;
Spartan_UserDisplayBusinessRuleButtons(Spartan_UserisdisplayBusinessPropery);
function Spartan_UserDisplayBusinessRule() {
    if (!Spartan_UserisdisplayBusinessPropery) {
        $('#CreateSpartan_User').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_UserdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_UserPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_UserdisplayBusinessPropery').remove();
    }
    Spartan_UserDisplayBusinessRuleButtons(!Spartan_UserisdisplayBusinessPropery);
    Spartan_UserisdisplayBusinessPropery = (Spartan_UserisdisplayBusinessPropery ? false : true);
}
function Spartan_UserDisplayBusinessRuleButtons(flag) {

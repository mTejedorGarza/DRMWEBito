...
function ClearControls() {
    $("#ReferenceOperatorTypeId").val("0");
    $('#CreateSpartan_BR_Operator_Type')[0].reset();
    ClearFormControls();
    $("#OperatorTypeIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Operator_Type').trigger('reset');
    $('#CreateSpartan_BR_Operator_Type').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Operator_Type');
...
function ResetClaveLabel() {
    $("#lblOperatorTypeId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Operator_Type").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Operator_TypeData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Operator_TypeData(function (currentId) {
                $("#OperatorTypeIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceOperatorTypeId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Operator_Type').find('.col-sm-8').each(function () {

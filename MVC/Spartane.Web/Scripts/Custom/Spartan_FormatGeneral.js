...
function GetSpartanFormatRelatedData() {
    var SpartanFormatRelatesData = GetSpartanFormatRelated();
    var form_data = new FormData();
    for (var i = 0; i < SpartanFormatRelatesData.length; i++) {
        form_data.append('[' + i + '].Clave', SpartanFormatRelatesData[i].Clave);
        form_data.append('[' + i + '].FormatId_Related', SpartanFormatRelatesData[i].FormatId_Related);
    }
    return form_data;
}


function GetSpartanFormatRelated() {
    var SpartanFormatRelatesData = [];
    var Data = $('#formats_related').val();
    if (Data != null) {
        for (var i = 0; i < Data.length; i++) {
            SpartanFormatRelatesData.push({
                Clave: 0
                    , FormatId_Related: Data[i]
            });
        }
    }
    return SpartanFormatRelatesData;
}
...
function ClearControls() {
    $("#ReferenceFormatId").val("0");
    $('#CreateSpartan_Format')[0].reset();
    ClearFormControls();
    $("#FormatIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Format').trigger('reset');
    $('#CreateSpartan_Format').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Format');
...
$(document).ready(function () {
    $("form#CreateSpartan_Format").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Format").on('click', '#Spartan_FormatCancelar', function () {
        BackToGrid();
    });
	$("form#CreateSpartan_Format").on('click', '#Spartan_FormatGuardar', function () {
        EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_FormatData(function () {
                EjecutarValidacionesDespuesDeGuardar();
                BackToGrid();
            });
    });
	$("form#CreateSpartan_Format").on('click', '#Spartan_FormatGuardarYNuevo', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation()) {
            SendSpartan_FormatData(function () {
                ClearControls();
    ...
    $("form#CreateSpartan_Format").on('click', '#Spartan_FormatGuardarYCopia', function () {
		EjecutarValidacionesAntesDeGuardar();
        if (CheckValidation())
            SendSpartan_FormatData(function (currentId) {
                ...
                ResetClaveLabel();
                $("#ReferenceFormatId").val(currentId);
...
var Spartan_FormatisdisplayBusinessPropery = false;
Spartan_FormatDisplayBusinessRuleButtons(Spartan_FormatisdisplayBusinessPropery);
function Spartan_FormatDisplayBusinessRule() {
    if (!Spartan_FormatisdisplayBusinessPropery) {
        $('#CreateSpartan_Format').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="displayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_FormatPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_FormatdisplayBusinessPropery').remove();
    }
    Spartan_FormatDisplayBusinessRuleButtons(!Spartan_FormatisdisplayBusinessPropery);
    Spartan_FormatisdisplayBusinessPropery = (Spartan_FormatisdisplayBusinessPropery ? false : true);
}
function Spartan_FormatDisplayBusinessRuleButtons(flag) {

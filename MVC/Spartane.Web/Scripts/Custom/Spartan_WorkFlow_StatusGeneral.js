...
function ClearControls() {
    ...
    $('#CreateSpartan_WorkFlow_Status')[0].reset();
    ClearFormControls();
    $("#StatusIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_WorkFlow_Status').trigger('reset');
    $('#CreateSpartan_WorkFlow_Status').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_WorkFlow_Status');
...
$(document).ready(function () {
    $("form#CreateSpartan_WorkFlow_Status").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_WorkFlow_Status").on('click', '#Spartan_WorkFlow_StatusCancelar', function () {
        Spartan_WorkFlow_StatusBackToGrid();
    });
	$("form#CreateSpartan_WorkFlow_Status").on('click', '#Spartan_WorkFlow_StatusGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_StatusData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_WorkFlow_StatusBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_WorkFlow_Status").on('click', '#Spartan_WorkFlow_StatusGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_WorkFlow_StatusData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_WorkFlow_Status").on('click', '#Spartan_WorkFlow_StatusGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_WorkFlow_StatusData(function (currentId) {
					$("#StatusIdId").val("0");
...
var Spartan_WorkFlow_StatusisdisplayBusinessPropery = false;
Spartan_WorkFlow_StatusDisplayBusinessRuleButtons(Spartan_WorkFlow_StatusisdisplayBusinessPropery);
function Spartan_WorkFlow_StatusDisplayBusinessRule() {
    if (!Spartan_WorkFlow_StatusisdisplayBusinessPropery) {
        $('#CreateSpartan_WorkFlow_Status').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_WorkFlow_StatusdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_WorkFlow_StatusPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_WorkFlow_StatusdisplayBusinessPropery').remove();
    }
    Spartan_WorkFlow_StatusDisplayBusinessRuleButtons(!Spartan_WorkFlow_StatusisdisplayBusinessPropery);
    Spartan_WorkFlow_StatusisdisplayBusinessPropery = (Spartan_WorkFlow_StatusisdisplayBusinessPropery ? false : true);
}
function Spartan_WorkFlow_StatusDisplayBusinessRuleButtons(flag) {

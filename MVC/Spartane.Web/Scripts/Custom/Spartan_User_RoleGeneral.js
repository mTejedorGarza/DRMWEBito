...
function ClearControls() {
    $("#ReferenceUser_Role_Id").val("0");
    $('#CreateSpartan_User_Role')[0].reset();
    ClearFormControls();
    $("#User_Role_IdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_User_Role').trigger('reset');
    $('#CreateSpartan_User_Role').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_User_Role');
...
function ResetClaveLabel() {
    $("#lblUser_Role_Id").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_User_Role").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_User_Role").on('click', '#Spartan_User_RoleCancelar', function () {
        Spartan_User_RoleBackToGrid();
    });
	$("form#CreateSpartan_User_Role").on('click', '#Spartan_User_RoleGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_User_RoleData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_User_RoleBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_User_Role").on('click', '#Spartan_User_RoleGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_User_RoleData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_User_Role").on('click', '#Spartan_User_RoleGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_User_RoleData(function (currentId) {
					$("#User_Role_IdId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceUser_Role_Id").val(currentId);
...
var Spartan_User_RoleisdisplayBusinessPropery = false;
Spartan_User_RoleDisplayBusinessRuleButtons(Spartan_User_RoleisdisplayBusinessPropery);
function Spartan_User_RoleDisplayBusinessRule() {
    if (!Spartan_User_RoleisdisplayBusinessPropery) {
        $('#CreateSpartan_User_Role').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_User_RoledisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_User_RolePropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_User_RoledisplayBusinessPropery').remove();
    }
    Spartan_User_RoleDisplayBusinessRuleButtons(!Spartan_User_RoleisdisplayBusinessPropery);
    Spartan_User_RoleisdisplayBusinessPropery = (Spartan_User_RoleisdisplayBusinessPropery ? false : true);
}
function Spartan_User_RoleDisplayBusinessRuleButtons(flag) {

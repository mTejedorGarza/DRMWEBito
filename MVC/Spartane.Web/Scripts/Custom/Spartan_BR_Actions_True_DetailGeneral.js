...
function ClearControls() {
    $("#ReferenceActionsTrueDetailId").val("0");
    $('#CreateSpartan_BR_Actions_True_Detail')[0].reset();
    ClearFormControls();
    $("#ActionsTrueDetailIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Actions_True_Detail').trigger('reset');
    $('#CreateSpartan_BR_Actions_True_Detail').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Actions_True_Detail');
...
function ResetClaveLabel() {
    $("#lblActionsTrueDetailId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Actions_True_Detail").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Actions_True_DetailData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Actions_True_DetailData(function (currentId) {
                $("#ActionsTrueDetailIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceActionsTrueDetailId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Actions_True_Detail').find('.col-sm-8').each(function () {

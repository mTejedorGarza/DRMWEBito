...
function ClearControls() {
    $("#ReferenceReportFilterId").val("0");
    $('#CreateSpartan_Report_Filter')[0].reset();
    ClearFormControls();
    $("#ReportFilterIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Report_Filter').trigger('reset');
    $('#CreateSpartan_Report_Filter').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Report_Filter');
...
function ResetClaveLabel() {
    $("#lblReportFilterId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_Report_Filter").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Report_Filter").on('click', '#Spartan_Report_FilterCancelar', function () {
        Spartan_Report_FilterBackToGrid();
    });
	$("form#CreateSpartan_Report_Filter").on('click', '#Spartan_Report_FilterGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Report_FilterData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_Report_FilterBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_Report_Filter").on('click', '#Spartan_Report_FilterGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_Report_FilterData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_Report_Filter").on('click', '#Spartan_Report_FilterGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Report_FilterData(function (currentId) {
					$("#ReportFilterIdId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceReportFilterId").val(currentId);
...
var Spartan_Report_FilterisdisplayBusinessPropery = false;
Spartan_Report_FilterDisplayBusinessRuleButtons(Spartan_Report_FilterisdisplayBusinessPropery);
function Spartan_Report_FilterDisplayBusinessRule() {
    if (!Spartan_Report_FilterisdisplayBusinessPropery) {
        $('#CreateSpartan_Report_Filter').find('.col-sm-8').each(function () {
			var div =$(this); 
            ...
            mainElementAttributes += '<div class="Spartan_Report_FilterdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Report_FilterPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Report_FilterdisplayBusinessPropery').remove();
    }
    Spartan_Report_FilterDisplayBusinessRuleButtons(!Spartan_Report_FilterisdisplayBusinessPropery);
    Spartan_Report_FilterisdisplayBusinessPropery = (Spartan_Report_FilterisdisplayBusinessPropery ? false : true);
}
function Spartan_Report_FilterDisplayBusinessRuleButtons(flag) {

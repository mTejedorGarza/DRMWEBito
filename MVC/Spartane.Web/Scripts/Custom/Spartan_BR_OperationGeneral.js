...
function ClearControls() {
    $("#ReferenceOperationId").val("0");
    $('#CreateSpartan_BR_Operation')[0].reset();
    ClearFormControls();
    $("#OperationIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Operation').trigger('reset');
    $('#CreateSpartan_BR_Operation').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Operation');
...
function ResetClaveLabel() {
    $("#lblOperationId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Operation").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_OperationData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_OperationData(function (currentId) {
                $("#OperationIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceOperationId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Operation').find('.col-sm-8').each(function () {

...
function ClearControls() {
    $("#ReferenceIdObject").val("0");
    $('#CreateSpartan_Traduction_Object')[0].reset();
    ClearFormControls();
    $("#IdObjectId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Traduction_Object').trigger('reset');
    $('#CreateSpartan_Traduction_Object').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Traduction_Object');
...
$(document).ready(function () {
    $("form#CreateSpartan_Traduction_Object").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Traduction_Object").on('click', '#Spartan_Traduction_ObjectCancelar', function () {
        Spartan_Traduction_ObjectBackToGrid();
    });
	$("form#CreateSpartan_Traduction_Object").on('click', '#Spartan_Traduction_ObjectGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Traduction_ObjectData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_Traduction_ObjectBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_Traduction_Object").on('click', '#Spartan_Traduction_ObjectGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_Traduction_ObjectData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_Traduction_Object").on('click', '#Spartan_Traduction_ObjectGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Traduction_ObjectData(function (currentId) {
					$("#IdObjectId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceIdObject").val(currentId);
...
var Spartan_Traduction_ObjectisdisplayBusinessPropery = false;
Spartan_Traduction_ObjectDisplayBusinessRuleButtons(Spartan_Traduction_ObjectisdisplayBusinessPropery);
function Spartan_Traduction_ObjectDisplayBusinessRule() {
    if (!Spartan_Traduction_ObjectisdisplayBusinessPropery) {
        $('#CreateSpartan_Traduction_Object').find('.col-sm-8').each(function () {
            var mainElementAttributes = '<div class="col-sm-2">';
            var fieldId = $(this).data('field-id');
            var fieldName = $(this).data('field-name');
            var attribute = $(this).data('attribute');
            mainElementAttributes += '<div class="Spartan_Traduction_ObjectdisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Traduction_ObjectPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Traduction_ObjectdisplayBusinessPropery').remove();
    }
    Spartan_Traduction_ObjectDisplayBusinessRuleButtons(!Spartan_Traduction_ObjectisdisplayBusinessPropery);
    Spartan_Traduction_ObjectisdisplayBusinessPropery = (Spartan_Traduction_ObjectisdisplayBusinessPropery ? false : true);
}
function Spartan_Traduction_ObjectDisplayBusinessRuleButtons(flag) {

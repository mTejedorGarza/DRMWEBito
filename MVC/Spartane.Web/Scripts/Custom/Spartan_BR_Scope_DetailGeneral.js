...
function ClearControls() {
    $("#ReferenceScopeDetailId").val("0");
    $('#CreateSpartan_BR_Scope_Detail')[0].reset();
    ClearFormControls();
    $("#ScopeDetailIdId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Scope_Detail').trigger('reset');
    $('#CreateSpartan_BR_Scope_Detail').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Scope_Detail');
...
function ResetClaveLabel() {
    $("#lblScopeDetailId").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Scope_Detail").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#Guardar").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Scope_DetailData(function () {
                BackToGrid();
            });
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSpartan_BR_Scope_DetailData(function (currentId) {
                $("#ScopeDetailIdId").val("0");

                ResetClaveLabel();
                $("#ReferenceScopeDetailId").val(currentId);
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Scope_Detail').find('.col-sm-8').each(function () {

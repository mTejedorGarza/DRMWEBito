...
function ClearControls() {
    ...
    $('#CreateSpartan_Traduction_Process_Data')[0].reset();
    ClearFormControls();
    $("#ClaveId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_Traduction_Process_Data').trigger('reset');
    $('#CreateSpartan_Traduction_Process_Data').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_Traduction_Process_Data');
...
$(document).ready(function () {
    $("form#CreateSpartan_Traduction_Process_Data").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_Traduction_Process_Data").on('click', '#Spartan_Traduction_Process_DataCancelar', function () {
        Spartan_Traduction_Process_DataBackToGrid();
    });
	$("form#CreateSpartan_Traduction_Process_Data").on('click', '#Spartan_Traduction_Process_DataGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Traduction_Process_DataData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_Traduction_Process_DataBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_Traduction_Process_Data").on('click', '#Spartan_Traduction_Process_DataGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_Traduction_Process_DataData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_Traduction_Process_Data").on('click', '#Spartan_Traduction_Process_DataGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_Traduction_Process_DataData(function (currentId) {
					$("#ClaveId").val("0");
...
var Spartan_Traduction_Process_DataisdisplayBusinessPropery = false;
Spartan_Traduction_Process_DataDisplayBusinessRuleButtons(Spartan_Traduction_Process_DataisdisplayBusinessPropery);
function Spartan_Traduction_Process_DataDisplayBusinessRule() {
    if (!Spartan_Traduction_Process_DataisdisplayBusinessPropery) {
        $('#CreateSpartan_Traduction_Process_Data').find('.col-sm-8').each(function () {
			var div =$(this); 
            ...
            mainElementAttributes += '<div class="Spartan_Traduction_Process_DatadisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_Traduction_Process_DataPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_Traduction_Process_DatadisplayBusinessPropery').remove();
    }
    Spartan_Traduction_Process_DataDisplayBusinessRuleButtons(!Spartan_Traduction_Process_DataisdisplayBusinessPropery);
    Spartan_Traduction_Process_DataisdisplayBusinessPropery = (Spartan_Traduction_Process_DataisdisplayBusinessPropery ? false : true);
}
function Spartan_Traduction_Process_DataDisplayBusinessRuleButtons(flag) {

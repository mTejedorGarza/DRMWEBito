...
function ClearControls() {
    $("#ReferenceKey_Rejection_Reason").val("0");
    $('#CreateSpartan_BR_Rejection_Reason')[0].reset();
    ClearFormControls();
    $("#Key_Rejection_ReasonId").val("0");
...
function ClearFormControls() {
    $('#CreateSpartan_BR_Rejection_Reason').trigger('reset');
    $('#CreateSpartan_BR_Rejection_Reason').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
function CheckValidation() {
    var $myForm = $('#CreateSpartan_BR_Rejection_Reason');
...
function ResetClaveLabel() {
    $("#lblKey_Rejection_Reason").text("0");
}
$(document).ready(function () {
    $("form#CreateSpartan_BR_Rejection_Reason").submit(function (e) {
        e.preventDefault();
        return false;
    });
	$("form#CreateSpartan_BR_Rejection_Reason").on('click', '#Spartan_BR_Rejection_ReasonCancelar', function () {
        Spartan_BR_Rejection_ReasonBackToGrid();
    });
	$("form#CreateSpartan_BR_Rejection_Reason").on('click', '#Spartan_BR_Rejection_ReasonGuardar', function () {
        if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_BR_Rejection_ReasonData(function () {
					EjecutarValidacionesDespuesDeGuardar();
					Spartan_BR_Rejection_ReasonBackToGrid();
				});
		}
    });
	$("form#CreateSpartan_BR_Rejection_Reason").on('click', '#Spartan_BR_Rejection_ReasonGuardarYNuevo', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation()) {
				SendSpartan_BR_Rejection_ReasonData(function () {
					ClearControls();
    ...
    $("form#CreateSpartan_BR_Rejection_Reason").on('click', '#Spartan_BR_Rejection_ReasonGuardarYCopia', function () {
		if (EjecutarValidacionesAntesDeGuardar()) {
			if (CheckValidation())
				SendSpartan_BR_Rejection_ReasonData(function (currentId) {
					$("#Key_Rejection_ReasonId").val("0");
	
					ResetClaveLabel();
					$("#ReferenceKey_Rejection_Reason").val(currentId);
...
var Spartan_BR_Rejection_ReasonisdisplayBusinessPropery = false;
Spartan_BR_Rejection_ReasonDisplayBusinessRuleButtons(Spartan_BR_Rejection_ReasonisdisplayBusinessPropery);
function Spartan_BR_Rejection_ReasonDisplayBusinessRule() {
    if (!Spartan_BR_Rejection_ReasonisdisplayBusinessPropery) {
        $('#CreateSpartan_BR_Rejection_Reason').find('.col-sm-8').each(function () {
			var div =$(this); 
            ...
            mainElementAttributes += '<div class="Spartan_BR_Rejection_ReasondisplayBusinessPropery"><button type="button" data-field-id="' + fieldId + '" data-field-name="' + fieldName + '" data-attribute="' + attribute + '" class="btn btn-info btn-lg btnPopupBusinessRules" data-toggle="modal" data-target="#Spartan_BR_Rejection_ReasonPropertyBusinessModal-form"><i class="fa fa-cogs fa-spin"></i></button></div>';
            mainElementAttributes += '</div>';
            $(this).after(mainElementAttributes);
        });
    } else {
        $('.Spartan_BR_Rejection_ReasondisplayBusinessPropery').remove();
    }
    Spartan_BR_Rejection_ReasonDisplayBusinessRuleButtons(!Spartan_BR_Rejection_ReasonisdisplayBusinessPropery);
    Spartan_BR_Rejection_ReasonisdisplayBusinessPropery = (Spartan_BR_Rejection_ReasonisdisplayBusinessPropery ? false : true);
}
function Spartan_BR_Rejection_ReasonDisplayBusinessRuleButtons(flag) {
